...
namespace main.contentslist
{
    public partial class List_AHUSystem : Form
    {
        ...
        {
            //Num = Program.UTIL.CreateNum("AHUSystem_Form", "번호", "HS");
        ...
        public void load_List()
        {
            ...
            if (List.Length > 0)
            {
                ...
                for (int n = 0; n < List.Length; n++)
                {
                    ...
                    if (List[n][2] == "공조기")
                    {
                        Value = Program.DB.getValue(DB.type.ProjDB, "User_AHU", "급기풍량,온도교환효율_난방,온도교환효율_냉방,난방코일출력,냉각코일출력", "번호='" + List[n][0] + "'");
                        if (Value.Length > 0)
                        {
                    ...
                    {
                        Value = Program.DB.getValue(DB.type.ProjDB, "User_HRV", "팬풍량,온도교환효율_난방,온도교환효율_냉방", "번호='" + List[n][0] + "'");
                        if (Value.Length > 0)
                        {
                            dataGridView1.Rows[nRow].Cells[4].Value = String.Format("{0:F0}", Convert.ToDouble(Value[0][0]));
                            dataGridView1.Rows[nRow].Cells[5].Value = String.Format("{0:F0}", Convert.ToDouble(Value[0][1]));
            ...
            CountDB = List.Length;
            Program.UTIL.resetMainTree(4, 1, mainMenu.ToArray(), "57");
