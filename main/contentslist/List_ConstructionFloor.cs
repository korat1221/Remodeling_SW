...
{
    public partial class List_ConstructionFloor : Form
    {
        ...
        private void Load_form(String ID, String editing)
        {
            currentID = ID;
            inEditing = editing;
            Program.getMenuForm().DoLoadForm(5, OnLoadProc);
        }
        ...
        public void load_List()
        {
            ...
            dataGridView1.DataSource = WallList;
            CountDB = List.Length;
            Program.UTIL.resetMainTree(1, 1, mainMenu.ToArray(), "36"); // 예시 코드: 메인 메뉴 동적 할당
        }
        ...
        private void Remove_button_Click(object sender, EventArgs e)
        {
            ...
            if ((MessageBox.Show(dataGridView1.Rows[k].Cells[2].Value.ToString() + "을 삭제 하시겠습니까?", "삭제 확인", MessageBoxButtons.YesNo) == DialogResult.Yes))
            {
                if (k > -1)
                {
                    ...
                    Program.DB.deleteValue(DB.type.ProjDB, "ConstructionFloor", "번호 ='" + Delete_Num + "'");
                    load_List();
