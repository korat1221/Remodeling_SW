...
namespace main
{
    ...
    internal class ZoneLight
    {
        ...
        public double Zone_f_Aca, Zone_f_aD, Zone_f_bD, Zone_f_AD, f_τD65_SNA, K1, K2, K3, γSh_lsh, γSh_hA, γSh_vA;
...

        {
            if (Main == "파사드")
            {
                ...
                Zone_IRD = general.Calc_IRD(Zone_f_aD, Zone_hLi, Zone_hTa);
                Console.WriteLine("투명도 계수 : " + Zone_ITr + "   " + "존 공간 계수 : " + Zone_IRD);
        ...
        public void Calc_Facade_FDS()
        {
            if (Main == "파사드")
            {
                ...
                for (int nn = 0; nn < 400; nn++)
                {
                    ...
                    if (파사드차양미가동주광공급계수테이블[nn, 0] == facade_di.ToString() && 파사드차양미가동주광공급계수테이블[nn, 1] == nearEm_SNA.ToString() && 파사드차양미가동주광공급계수테이블[nn, 2] == Zone_nearD.ToString())

                        find_fd_sna = Convert.ToDouble(파사드차양미가동주광공급계수테이블[nn, 3]);
                }
...

        {
            ...
            if (Main == "천창")
            {
                ...
                Console.WriteLine("as/bs :" + " " + Zone_as_bs + "  " + "hs/bs :" + " " + Zone_hs_bs + "  " + "hg/hw :" + " " + Zone_hg_hw);
        ...
        public void Calc_W()
        {
            ...
            if (Sub == "집광채광 O")
            {
                ...
                for (int i = 0; i < 12; i++)
                {
                    Zone_Final_W[i] = final_w.Calc_W_re_yes(Fc, Zone_Sunlight_PjSC[i], Pj, Fo, daytime[i], Zone_Facade_FD[i], Zone_Roof_FD[i], nighttime[i], wsp, A);
                    Console.WriteLine((i + 1) + "월 조명에너지 소요량 : " + "  " + Zone_Final_W[i]);
                }
            ...
            else if (Sub == "집광채광 X")
            {
                ...
                for (int i = 0; i < 12; i++)
                {
                    Zone_Final_W[i] = final_w.Calc_W_re_no(Fc, Pj, Fo, daytime[i], Zone_Facade_FD[i], Zone_Roof_FD[i], nighttime[i], wsp, A);
                    Console.WriteLine((i + 1) + "월 조명에너지 소요량 : " + "  " + Zone_Final_W[i]);
                }
            }
