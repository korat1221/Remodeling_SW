...
namespace main.subcontents.HeatingSystem
{
    public partial class Heating_ceZone : Form
    {
        ...
        ArrayList SelectZone_split = new ArrayList(), SelectAHU_split = new ArrayList();
        ...
        void load_table_DB()
        {
            ...
            ceZone_dataGridView.Columns.Add("A5", "천장고");
            ...
            {
                String[][] ZoneValve = Program.DB.getValue(DB.type.ProjDB, "ZoneGeneral_Form", "존번호,존이름,용도프로필,천장고", "존번호 = '" + SelectZone_split[n].ToString() + "'");
                ...
                if (ZoneValve.Length > 0)
                {
                    ...
                    ceZone_dataGridView.Rows[n].Cells[5].Value = ZoneValve[0][3]; //천장고
                    DataGridViewButtonCell ButtonCell = new DataGridViewButtonCell();
                    ceZone_dataGridView.Rows[n].Cells[7] = ButtonCell;//적용 
                    ButtonCell.Value = "+";
                }
        ...
        {
            if (ceZone_dataGridView.Rows[row].Cells[7].GetType() == typeof(DataGridViewButtonCell))
            {
                if (column == 6 && cell.Value == null)
                {
        ...
        private void ceZone_dataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                if (e.ColumnIndex == 7)
                {
                    ...
                    int 대수 = Convert.ToInt16(Program.UTIL.dataGridView_doubleComa(ceZone_dataGridView, e.RowIndex, 6, 0));
                    for (int k = (대수 - 1); k > -1; k--)
                    {
                        ...
                        {
                            String[][] 일람표 = Program.DB.getValue(DB.type.ProjDB, "User_ce", "명칭", "유형 = '천장고4m초과' AND 구분 = '" + ceType + "' AND 난방냉방 !='냉방'");
                            if (일람표.Length > 0)
                            {
                                for (int j = 0; j < 일람표.Length; j++)
                                {
        ...
        {
            String[][] Size = Program.DB.getValue(DB.type.ProjDB, "Heating_ce_Form", "존번호", "난방시스템 = '" + SystemNum + "' And 구분 = '" + ceType + "'");
            if (Size.Length > 0)
            {
                Program.DB.deleteValue(DB.type.ProjDB, "Heating_ce_Form", "난방시스템 = '" + SystemNum + "' And 구분 = '" + ceType + "'");
            }
            ...
            for (int i = 0; i < ceZone_dataGridView.Rows.Count; i++)
            {
                ...
                if (index > 0)
                {
                    ...
                    string[][] 공급설비일람표 = Program.DB.getValue(DB.type.ProjDB, "User_ce", "유형,구분,종류,번호", "명칭 = '" + ceZone_dataGridView.Rows[i].Cells[8].Value + "'");
                    if (공급설비일람표.Length > 0)
                    {
                        Program.DB.setValue(DB.type.ProjDB, "Heating_ce_Form", "존번호,프로젝트유형,난방시스템,유형,구분,종류,번호",
                         "'" + 존번호 + "','" + 프로젝트유형[0][0] + "','"+ SystemNum + "','"+ 공급설비일람표[0][0] + "','" + 공급설비일람표[0][1] + "','" + 공급설비일람표[0][2] + "','" + 공급설비일람표[0][3] + "_" + substring2 + "'", "");
                    }
                }
            }
