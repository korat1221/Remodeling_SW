    ...
    public partial class Cooling_SoilCooler : Form
    {
        ...
        private void load_table_DB() //번호자동생성, 
        {
            ...
            for (int i = 0; i < DefaultDB_Value.Length; i++)
            {
                ...
                SoilCooler_dataGridView.Rows[nRow].Cells[4].Value = string.Format("{0:F1}", Convert.ToDouble(DefaultDB_Value[i][2]));//냉방출력
                SoilCooler_dataGridView.Rows[nRow].Cells[5].Value = string.Format("{0:F1}", Convert.ToDouble(DefaultDB_Value[i][3]));//냉방소비전력
                SoilCooler_dataGridView.Rows[nRow].Cells[6].Value = string.Format("{0:F1}", Convert.ToDouble(DefaultDB_Value[i][4])); //EER
