...
namespace main.subcontents.CoolingSystem
{
    public partial class Cooling_SoilCooler : Form
    {
        ...
        public Cooling_SoilCooler(string _Num, string _SelectCG_nonsplit)
        {
        ...
        private void load_table_DB() //번호자동생성, 
        {
            ...
            for (int i = 0; i < DefaultDB_Value.Length; i++)
            {
                ...
                SoilCooler_dataGridView.Rows[nRow].Cells[1].Value = DefaultDB_Value[i][0];//번호
                SoilCooler_dataGridView.Rows[nRow].Cells[2].Value = DefaultDB_Value[i][1]; //명칭
                SoilCooler_dataGridView.Rows[nRow].Cells[3].Value = string.Format("{0:F1}", Program.UTIL.ToDoubleOrZero(DefaultDB_Value[i][2]));//냉방출력
                SoilCooler_dataGridView.Rows[nRow].Cells[4].Value = string.Format("{0:F1}", Program.UTIL.ToDoubleOrZero(DefaultDB_Value[i][3]));//냉방소비전력
                SoilCooler_dataGridView.Rows[nRow].Cells[5].Value = string.Format("{0:F1}", Program.UTIL.ToDoubleOrZero(DefaultDB_Value[i][4])); //EER
                SoilCooler_dataGridView.Rows[nRow].Cells[6].Value = DefaultDB_Value[i][5]; //대기전력
                SoilCooler_dataGridView.Rows[nRow].Cells[7].Value = DefaultDB_Value[i][6]; //연료
                SoilCooler_dataGridView.Rows[nRow].Cells[8].Value = DefaultDB_Value[i][7]; //압축기
                SoilCooler_dataGridView.Rows[nRow].Cells[9].Value = DefaultDB_Value[i][8]; //설치
                SoilCooler_dataGridView.Rows[nRow].Cells[10].Value = DefaultDB_Value[i][9]; //냉수입구온도
                SoilCooler_dataGridView.Rows[nRow].Cells[11].Value = DefaultDB_Value[i][10]; //냉수출구온도
            }
        }
        private void TableMake()
        {
            ...
            SoilCooler_dataGridView.Columns.Add("A3", "냉방성능.출력.[kW]");
            SoilCooler_dataGridView.Columns.Add("A4", "냉방성능.소비전력.[kW]");
            SoilCooler_dataGridView.Columns.Add("A5", "냉방성능.EER.[W/W]");
            SoilCooler_dataGridView.Columns.Add("A6", "대기전력.[W]");
            SoilCooler_dataGridView.Columns.Add("A7", "연료");
            SoilCooler_dataGridView.Columns.Add("A8", "압축기");
            SoilCooler_dataGridView.Columns.Add("A9", "설치");
            SoilCooler_dataGridView.Columns.Add("A10", "냉수온도.입구.[℃]");
        ...
        private void Save_button_Click(object sender, EventArgs e)
        {
            ...
            for (int k = 0; k < SelectRow.Count; k++)
            {
                if (k == SelectRow.Count - 1)
                {
                    ...
                    this.SelectCG += SoilCooler_dataGridView.Rows[Convert.ToInt32(SelectRow[k])].Cells[1].Value.ToString();
        ...
        private void Load_SaveValue(string _SelectCG_nonsplit)
        {
            ...
            for (int k = 0; k < SelectCG_split.Count; k++)
            {
                for (int n = 0; n < SoilCooler_dataGridView.Rows.Count; n++)
                {
                    if (SoilCooler_dataGridView.Rows[n].Cells[1].Value.ToString() == SelectCG_split[k].ToString())
                    {
