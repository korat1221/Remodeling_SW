...
namespace main.subcontents.CoolingSystem
{
    public partial class Cooling_WaterCooler : Form
    {
        ...
        public Cooling_WaterCooler(string _Num, string _SelectCG_nonsplit)
        {
        ...
        private void load_table_DB() //번호자동생성, 
        {
            ...
            for (int i = 0; i < DefaultDB_Value.Length; i++)
            {
                ...
                WaterCooler_dataGridView.Rows[nRow].Cells[1].Value = DefaultDB_Value[i][0];//번호
                WaterCooler_dataGridView.Rows[nRow].Cells[2].Value = DefaultDB_Value[i][1]; //명칭
                WaterCooler_dataGridView.Rows[nRow].Cells[3].Value = string.Format("{0:F1}", Program.UTIL.ToDoubleOrZero(DefaultDB_Value[i][2]));//냉방출력
                WaterCooler_dataGridView.Rows[nRow].Cells[4].Value = string.Format("{0:F1}", Program.UTIL.ToDoubleOrZero(DefaultDB_Value[i][3]));//냉방소비전력
                WaterCooler_dataGridView.Rows[nRow].Cells[5].Value = string.Format("{0:F1}", Program.UTIL.ToDoubleOrZero(DefaultDB_Value[i][4])); //EER
                WaterCooler_dataGridView.Rows[nRow].Cells[6].Value = DefaultDB_Value[i][5]; //대기전력
                WaterCooler_dataGridView.Rows[nRow].Cells[7].Value = DefaultDB_Value[i][6]; //연료
                WaterCooler_dataGridView.Rows[nRow].Cells[8].Value = DefaultDB_Value[i][7]; //압축기
                WaterCooler_dataGridView.Rows[nRow].Cells[9].Value = DefaultDB_Value[i][8]; //설치
                WaterCooler_dataGridView.Rows[nRow].Cells[10].Value = DefaultDB_Value[i][9]; //냉수입구온도
                WaterCooler_dataGridView.Rows[nRow].Cells[11].Value = DefaultDB_Value[i][10]; //냉수출구온도
            }
        }
        private void TableMake()
        {
            ...
            WaterCooler_dataGridView.Columns.Add("A6", "대기전력.[W]");
            WaterCooler_dataGridView.Columns.Add("A7", "연료");
            WaterCooler_dataGridView.Columns.Add("A8", "압축기");
            WaterCooler_dataGridView.Columns.Add("A9", "설치");
            WaterCooler_dataGridView.Columns.Add("A10", "냉수온도.입구.[℃]");
        ...
        private void Save_button_Click(object sender, EventArgs e)
        {
            ...
            for (int k = 0; k < SelectRow.Count; k++)
            {
                if (k == SelectRow.Count - 1)
                {
                    ...
                    this.SelectCG += WaterCooler_dataGridView.Rows[Convert.ToInt32(SelectRow[k])].Cells[1].Value.ToString();
