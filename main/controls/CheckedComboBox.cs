...
public class CheckedComboBox : System.Windows.Forms.ComboBox {
    ...
    internal class Dropdown : Form {
        ...
        {
            for (int i = 1; i < cclb.Items.Count; i++)
            {
                if (!cclb.GetItemChecked(i)) return false;
            }
            return true;
        }
        ...
        public string GetCheckedItemsStringValue() {
            ...
            else
            {
                ...
                while(++i < cclb.CheckedItems.Count)
                {
                    if (s != "") s += ",";

                    s2 = cclb.GetItemText(cclb.CheckedItems[i]);
                    if (s2 != "All") s += s2;
                }
                return s;
            }
...
//            this.Hide();
        ...
        public void SetAllItemChecked(CheckState state)
        {
            for (int i = 1; i < ccbParent.Items.Count; i++)
            {
                ccbParent.SetItemChecked(i, state == CheckState.Checked, true);
            }
        }
  ...
  //      if (!dropdown.Visible) {
    ...
    public void SetItemChecked(int index, bool isChecked, bool isInternal = false) {
        if (index < 0 || index > Items.Count) {
            throw new ArgumentOutOfRangeException("index", "value out of range");
        } else {
    ...
    public bool IsChecked(String sText)
    {
        int i = -1;

        while (++i < Items.Count)
        {
            if (((CCBoxItem)Items[i]).CompareText(sText))
            {
                return dropdown.List.GetItemChecked(i);
            }
        }
        return false;
    }
    public void Add(String sText)
    {
        ...
        Items.Add(new CCBoxItem(sText, i));
