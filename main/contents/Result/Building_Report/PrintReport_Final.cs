    ...
    public partial class PrintReport_Final : Form
    {
        ...
        private void Error_Report()
        {
            ...
            while (++i < 번호.Length)
            {
                ...
                string[][] 연도2 = Program.DB.getValue_SameCheck(DB.type.ProjDB, "BuildingEnergyUse", "연도", "not 연료='전기' and not 연료='전체' ");
                try
                {
                    if (연도2.Length > 0)
                    {
                        __data[50].Add(new { idx = i, val = 연도2[0][0] + "년 가스 에너지사용량" }); //연도 표기 
                        __data[53].Add(new { idx = i, val = 연도2[0][0] + "년 가스 에너지사용량" }); //연도 표기 
                        year_gas[0] = 연도2[0][0];

                        __data[51].Add(new { idx = i, val = 연도2[1][0] + "년 가스 에너지사용량" }); //연도 표기 
                        __data[54].Add(new { idx = i, val = 연도2[1][0] + "년 가스 에너지사용량" }); //연도 표기 
                        year_gas[1] = 연도2[1][0];

                        __data[52].Add(new { idx = i, val = 연도2[2][0] + "년 가스 에너지사용량" }); //연도 표기 
                        __data[55].Add(new { idx = i, val = 연도2[2][0] + "년 가스 에너지사용량" }); //연도 표기 
                        year_gas[2] = 연도2[2][0];
                    }
                }
                catch { }
                string[][] Value_사용시작일_가스 = Program.DB.getValue(DB.type.ProjDB, "BuildingEnergyUse", "사용시작일", "not 연료='전기' and not 연료='전체'  AND 단위 ='kWh'");
                int yearnum_가스 = 0;
                ...
                for (int mth = 0; mth < 12; mth++)
                {
                    string[][] Final = Program.DB.getValue(DB.type.ProjDB, "FinalEnergy_Result", "난방,냉방,급탕,조명,공조,기저에너지,연료", "not 연료='전기' and not 연료='전체'  and 월 ='" + (mth + 1).ToString() + "월'");
                    if(Final.Length > 0)
                    {
                        Carrier = Final[0][6];
                ...
                string[][] check2 = Program.DB.getValue(DB.type.ProjDB, "BuildingEnergyUse", "에너지사용량", "not 연료 ='전기'");
                if(check2.Length > 0 )
                {
            ...
            string[][] value = Program.DB.getValue(DB.type.ProjDB, "FinalEnergy_Result", "난방,냉방,급탕,조명,공조,기저에너지", "not 연료='전기' and not 연료='전체'  and 월 ='" + (1).ToString() + "월'");
            if (value.Length > 0)
            {
            ...
            Debug.Print("start");
            string[][] check = Program.DB.getValue(DB.type.ProjDB, "BuildingEnergyUse", "에너지사용량", "not 연료 ='전기'");
                ...
                if (res.Length > 0)
                {
                    ...
                    { //리모델링전 전기 소요량 
                        ...
                        if(Final.Length >0 )
                        {
                            ...
                            __data[100].Add(new { idx = i * 12 + mth, val = Convert.ToDouble(Final[0][6]).ToString("0.0") }); //월별 신재생 생산량 
                            Qtot2_mth_전기[mth] = Convert.ToDouble(Final[0][7]);
                            ...
                            Qreg_a2_전기 += Convert.ToDouble(Final[0][6]);
                        }
                    }
                    ...
                    { //리모델링후 전기 소요량 
                        ...
                        if(Final.Length >0 && Final2.Length >0)
                        {
                            ...
                            __data[26].Add(new { idx = i * 12 + mth, val = Convert.ToDouble(Final2[0][0]).ToString("0.0") }); //월별 기저 >>>리모델링 전 값 가져옴 
                           
                            __data[103].Add(new { idx = i * 12 + mth, val = Convert.ToDouble(Final[0][6]).ToString("0.0") }); //월별 신재생 생산량 
                            Qtot_mth_전기[mth] = Convert.ToDouble(Final[0][7]);
                            ...
                            Ql_a_전기 += Convert.ToDouble(Final[0][3]);
                            Qv_a_전기 += Convert.ToDouble(Final[0][4]);
                            Qbase_a_전기 += Convert.ToDouble(Final2[0][0]); //리모델링전 값 가져옴 
                            Qreg_a_전기 += Convert.ToDouble(Final[0][6]);
                        }                       
                    }
                    Qtot_a_전기 = Qh_a_전기 + Qc_a_전기 + Qw_a_전기 + Ql_a_전기 + Qv_a_전기 + Qbase_a_전기 - Qreg_a_전기;
                    ...
                    for (int mth = 0; mth < 12; mth++)
                    { //리모델링전 가스 소요량 
                        string[][] Final = Program.DB.querySQL(res[0][0], "SELECT 난방,냉방,급탕,조명,공조,기저에너지,연료 FROM FinalEnergy_Result where not 연료='전기' and not 연료='전체'  and 월 = '" + (mth + 1).ToString() + "월'");
                    ...
                    for (int mth = 0; mth < 12; mth++)
                    { //리모델링후 가스 소요량 
                        string[][] Final = Program.DB.getValue(DB.type.ProjDB, "FinalEnergy_Result", "난방,냉방,급탕,조명,공조,기저에너지", "not 연료='전기' and not 연료='전체' and 월 ='" + (mth + 1).ToString() + "월'");
                        string[][] Final2 = Program.DB.querySQL(res[0][0], "SELECT 기저에너지 FROM FinalEnergy_Result where not 연료='전기' and not 연료='전체' and 월 = '" + (mth + 1).ToString() + "월'");
                        if(Final.Length >0 && Final2.Length >0)
                        {
                            ...
                            __data[76].Add(new { idx = i * 12 + mth, val = Convert.ToDouble(Final2[0][0]).ToString("0.0") }); //월별 기저 
                            Qtot_mth_가스[mth] = Convert.ToDouble(Final[0][0]) + Convert.ToDouble(Final[0][1]) + Convert.ToDouble(Final[0][2]) + Convert.ToDouble(Final[0][3]) + Convert.ToDouble(Final[0][4]) + Convert.ToDouble(Final2[0][0]);
                            ...
                            Ql_a_가스 += Convert.ToDouble(Final[0][3]);
                            Qv_a_가스 += Convert.ToDouble(Final[0][4]);
                            Qbase_a_가스 += Convert.ToDouble(Final2[0][0]);
                        }                        
                    }
                    Qtot_a_가스 = Qh_a_가스 + Qc_a_가스 + Qw_a_가스 + Ql_a_가스 + Qv_a_가스 + Qbase_a_가스;
