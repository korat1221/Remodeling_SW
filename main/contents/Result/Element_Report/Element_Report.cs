...
namespace main.contents
{
    public partial class Element_Report : Form
    {
        ...
        public void load_List()
        {
            ...
            MainMenu.Add(new { text = "불투명구조체", id = "{\\\"formID\\\":58,\\\"ID\\\":\\\"Result_1\\\"}" }); // 예시 코드: 메인 메뉴 동적 할당
            MainMenu.Add(new { text = "투명구조체", id = "{\\\"formID\\\":59,\\\"ID\\\":\\\"Result_2\\\"}" }); // 예시 코드: 메인 메뉴 동적 할당
        ...
        private void Report_After()
        {
            ...
            while (++i < 번호.Length)
            {
                if (res.Length > 0)
                {
                    ...
                    string[][] Value = Program.DB.querySQL(DB.type.ProjDB, "SELECT DISTINCT a.명칭,a.유효열관류율,a.기존외벽,a.번호 From ConstructionWall as a  Inner Join ZoneEnvelope_3D as b  on a.번호 = b.구조체번호  where b.외피유형 ='외벽' Order by a.유효열관류율 DESC");
                    ...
                    if (Value.Length > 0)
                    {
                       for(int k =0; k < Value.Length; k++)
                        ...
                        Wall_data[20].Add(new { idx = i, val = wall_area_sum.ToString("0.0") });//면적합계
                        Wall_data[21].Add(new { idx = i, val = "100 %" });//면적율합계
                        data.Add(new { cname = "wall_area_sum", data = Wall_data[20] });
                        data.Add(new { cname = "wall_area_sum_percent", data = Wall_data[21] });

                        for (int a = 0; a < 8; a++)
                        {
                            if (wall_name[a] != null && wall_name[a] != "")
                            {
                                Wall_data[22 + a].Add(new { idx = i, val = (wall_area[a] / wall_area_sum * 100).ToString("0") + " %" });//면적율
                                data.Add(new { cname = "wall_area_percent" + a, data = Wall_data[22 + a] });
                        ...
                        for (int a = 0; a < 8; a++)
                        {
                            if (wall_name[a] != null && wall_name[a]!="")
                            {
                                Wall_data[30 + a].Add(new { idx = i, val = wall_ueff[a].ToString("0.00") });//계획열관류율
                                data.Add(new { cname = "wall_ueff" + a, data = Wall_data[30 + a] });
                                Wall_data[38 + a].Add(new { idx = i, val = wall_ueff_old[a].ToString("0.00") });//기존열관류율
                                data.Add(new { cname = "wall_ueff_old" + a, data = Wall_data[38 + a] });
                        ...
                        Wall_data[39].Add(new { idx = i, val = wall_ueff_avg.ToString("0.00") });//계획열관류율 평균
                        Wall_data[40].Add(new { idx = i, val = wall_ueff_old_avg.ToString("0.00") });//기존열관류율 평균
                        data.Add(new { cname = "wall_ueff_avg", data = Wall_data[39] });
                        data.Add(new { cname = "wall_ueff_old_avg", data = Wall_data[40] });
                        ...
                        for (int a = 0; a < 8; a++)
                        {
                            if (wall_name[a]!=null && wall_name[a]!="")
                            {
                                if (sum != 0)
                                {
                                    Wall_data[48 + a].Add(new { idx = i, val = ((wall_saving / Total_Energy_pre) * (wall_saving_element[a] / sum) * 100).ToString("0.0") + " %" });//요소기술별 에너지절감률
                                }
                                else
                                {
                                    Wall_data[48 + a].Add(new { idx = i, val = (0).ToString("0.0") + " %" });//요소기술별 에너지절감률
                                }
                                data.Add(new { cname = "wall_saving_element" + a, data = Wall_data[48 + a] });
                            }
                        }
                        Wall_data[56].Add(new { idx = i, val = (wall_saving / Total_Energy_pre * 100).ToString("0.0") + " %" });  //요소기술별 절감률 합계
                        data.Add(new { cname = "wall_saving_element_sum", data = Wall_data[56] });
                        ...
                        for (int a = 0; a < 8; a++)
                        {
                            if (wall_name[a] != null && wall_name[a] != "")
                            {
                                ...
                                if(value2.Length > 0)
                                {
                                    Wall_data[57 + a].Add(new { idx = i, val = (Convert.ToDouble(value2[0][0])/wall_ueff[a] * 100).ToString("0") + " 점" });//법규대비 성능점수
                                    data.Add(new { cname = "wall_law_point" + a, data = Wall_data[57 + a] });
                                    wall_law_avg += Convert.ToDouble(value2[0][0]) * wall_area[a] / wall_area_sum;
                                }
                            }
                        }
                        Wall_data[58].Add(new { idx = i, val = (wall_law_avg / wall_ueff_avg * 100).ToString("0") + " 점" });//법규대비 성능점수 평균
                        data.Add(new { cname = "wall_law_point_avg", data = Wall_data[58] });                       
                    ...
                    Value = Program.DB.querySQL(DB.type.ProjDB, "SELECT DISTINCT a.명칭,a.유효열관류율,a.기존지붕,a.번호 From ConstructionRoof as a  Inner Join ZoneEnvelope_3D as b  on a.번호 = b.구조체번호  where b.외피유형 ='지붕' Order by a.유효열관류율 DESC");
                    string[] roof_num = new string[8]; string[] roof_name = new string[8]; double[] roof_ueff = new double[8]; double[] roof_ueff_old = new double[8]; double[] roof_area = new double[8]; double[] roof_saving_element = new double[8];
                    ...
                    if (Value.Length > 0)
                    {
                        ...
                        Roof_data[20].Add(new { idx = i, val = roof_area_sum.ToString("0.0") });//면적합계
                        Roof_data[21].Add(new { idx = i, val = "100 %" });//면적율합계
                        data.Add(new { cname = "roof_area_sum", data = Roof_data[20] });
                        data.Add(new { cname = "roof_area_sum_percent", data = Roof_data[21] });

                        for (int a = 0; a < 8; a++)
                        {
                            if (roof_name[a] != null && roof_name[a] != "")
                            {
                                Roof_data[22 + a].Add(new { idx = i, val = (roof_area[a] / roof_area_sum * 100).ToString("0") + " %" });//면적율
                                data.Add(new { cname = "roof_area_percent" + a, data = Roof_data[22 + a] });
                        ...
                        for (int a = 0; a < 8; a++)
                        {
                            if (roof_name[a] != null && roof_name[a] != "")
                            {
                                Roof_data[30 + a].Add(new { idx = i, val = roof_ueff[a].ToString("0.00") });//계획열관류율
                                data.Add(new { cname = "roof_ueff" + a, data = Roof_data[30 + a] });
                                Roof_data[38 + a].Add(new { idx = i, val = roof_ueff_old[a].ToString("0.00") });//기존열관류율
                                data.Add(new { cname = "roof_ueff_old" + a, data = Roof_data[38 + a] });
                        ...
                        Roof_data[39].Add(new { idx = i, val = roof_ueff_avg.ToString("0.00") });//계획열관류율 평균
                        Roof_data[40].Add(new { idx = i, val = roof_ueff_old_avg.ToString("0.00") });//기존열관류율 평균
                        data.Add(new { cname = "roof_ueff_avg", data = Roof_data[39] });
                        data.Add(new { cname = "roof_ueff_old_avg", data = Roof_data[40] });
                        ...
                        for (int a = 0; a < 8; a++)
                        {
                            if (roof_name[a] != null && roof_name[a] != "")
                            {
                                if (sum != 0)
                                {
                                    Roof_data[48 + a].Add(new { idx = i, val = ((roof_saving / Total_Energy_pre) * (roof_saving_element[a] / sum) * 100).ToString("0.0") + " %" });//요소기술별 에너지절감률
                                }
                                else
                                {
                                    Roof_data[48 + a].Add(new { idx = i, val = (0).ToString("0.0") + " %" });//요소기술별 에너지절감률
                                }
                                data.Add(new { cname = "roof_saving_element" + a, data = Roof_data[48 + a] });
                            }
                        }
                        Roof_data[56].Add(new { idx = i, val = (roof_saving / Total_Energy_pre * 100).ToString("0.0") + " %" });  //요소기술별 절감률 합계
                        data.Add(new { cname = "roof_saving_element_sum", data = Roof_data[56] });
                        ...
                        for (int a = 0; a < 8; a++)
                        {
                            if (roof_name[a] != null && roof_name[a] != "")
                            {
                                ...
                                if (value2.Length > 0)
                                {
                                    Roof_data[57 + a].Add(new { idx = i, val = (Convert.ToDouble(value2[0][0]) / roof_ueff[a] * 100).ToString("0") + " 점" });//법규대비 성능점수
                                    data.Add(new { cname = "roof_law_point" + a, data = Roof_data[57 + a] });
                                    roof_law_avg += Convert.ToDouble(value2[0][0]) * roof_area[a] / roof_area_sum;
                                }
                            }
                        }
                        Roof_data[58].Add(new { idx = i, val = (roof_law_avg / roof_ueff_avg * 100).ToString("0") + " 점" });//법규대비 성능점수 평균
                        data.Add(new { cname = "roof_law_point_avg", data = Roof_data[58] });
                    ...
                    Value = Program.DB.querySQL(DB.type.ProjDB, "SELECT DISTINCT a.명칭,a.유효열관류율,a.기존바닥,a.번호 From ConstructionFloor as a  Inner Join ZoneEnvelope_3D as b  on a.번호 = b.구조체번호  where b.외피유형 ='최하층바닥' Order by a.유효열관류율 DESC");
                    string[] floor_num = new string[8]; string[] floor_name = new string[8]; double[] floor_ueff = new double[8]; double[] floor_ueff_old = new double[8]; double[] floor_area = new double[8]; double[] floor_saving_element = new double[8];
                    ...
                    if (Value.Length > 0)
                    {
                        ...
                        Floor_data[20].Add(new { idx = i, val = floor_area_sum.ToString("0.0") });//면적합계
                        Floor_data[21].Add(new { idx = i, val = "100 %" });//면적율합계
                        data.Add(new { cname = "floor_area_sum", data = Floor_data[20] });
                        data.Add(new { cname = "floor_area_sum_percent", data = Floor_data[21] });

                        for (int a = 0; a < 8; a++)
                        {
                            if (floor_name[a] != null && floor_name[a] != "")
                            {
                                Floor_data[22 + a].Add(new { idx = i, val = (floor_area[a] / floor_area_sum * 100).ToString("0") + " %" });//면적율
                                data.Add(new { cname = "floor_area_percent" + a, data = Floor_data[22 + a] });
                        ...
                        for (int a = 0; a < 8; a++)
                        {
                            if (floor_name[a] != null && floor_name[a] != "")
                            {
                                Floor_data[30 + a].Add(new { idx = i, val = floor_ueff[a].ToString("0.00") });//계획열관류율
                                data.Add(new { cname = "floor_ueff" + a, data = Floor_data[30 + a] });
                                Floor_data[38 + a].Add(new { idx = i, val = floor_ueff_old[a].ToString("0.00") });//기존열관류율
                                data.Add(new { cname = "floor_ueff_old" + a, data = Floor_data[38 + a] });
                        ...
                        Floor_data[39].Add(new { idx = i, val = floor_ueff_avg.ToString("0.00") });//계획열관류율 평균
                        Floor_data[40].Add(new { idx = i, val = floor_ueff_old_avg.ToString("0.00") });//기존열관류율 평균
                        data.Add(new { cname = "floor_ueff_avg", data = Floor_data[39] });
                        data.Add(new { cname = "floor_ueff_old_avg", data = Floor_data[40] });
                        ...
                        for (int a = 0; a < 8; a++)
                        {
                            if (floor_name[a] != null && floor_name[a] != "")
                            {
                                if (sum != 0)
                                {
                                    Floor_data[48 + a].Add(new { idx = i, val = ((floor_saving / Total_Energy_pre) * (floor_saving_element[a] / sum) * 100).ToString("0.0") + " %" });//요소기술별 에너지절감률
                                }
                                else
                                {
                                    Floor_data[48 + a].Add(new { idx = i, val = (0).ToString("0.0") + " %" });//요소기술별 에너지절감률
                                }
                                data.Add(new { cname = "floor_saving_element" + a, data = Floor_data[48 + a] });
                            }
                        }
                        Floor_data[56].Add(new { idx = i, val = (floor_saving / Total_Energy_pre * 100).ToString("0.0") + " %" });  //요소기술별 절감률 합계
                        data.Add(new { cname = "floor_saving_element_sum", data = Floor_data[56] });
                        ...
                        for (int a = 0; a < 8; a++)
                        {
                            if (floor_name[a] != null && floor_name[a] != "")
                            {
                                ...
                                if (value2.Length > 0)
                                {
                                    Floor_data[57 + a].Add(new { idx = i, val = (Convert.ToDouble(value2[0][0]) / floor_ueff[a] * 100).ToString("0") + " 점" });//법규대비 성능점수
                                    data.Add(new { cname = "floor_law_point" + a, data = Floor_data[57 + a] });
                                    floor_law_avg += Convert.ToDouble(value2[0][0]) * floor_area[a] / floor_area_sum;
                                }
                            }
                        }
                        Floor_data[58].Add(new { idx = i, val = (floor_law_avg / floor_ueff_avg * 100).ToString("0") + " 점" });//법규대비 성능점수 평균
                        data.Add(new { cname = "floor_law_point_avg", data = Floor_data[58] });
