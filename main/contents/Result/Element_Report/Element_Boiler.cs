...
{
    public partial class Element_Boiler : Form
    {
        bool scriptable = false;
        public Element_Boiler()
        {
        ...
        public string Report_Before()
        {
            ...
            while (++i < 번호.Length)
            {
                ...
                string[][] value = Program.DB.querySQL(DB.type.ProjDB, "Select 총에너지소요량 From FinalEnergy_Result_Rule Where 월='연간' and 연료='전체' and 검토유형='보일러'");
                string[][] value3 = Program.DB.getValue(DB.type.ProjDB, "FinalEnergy_Result", "총에너지소요량", "월='연간' and 연료='전체'");
                double boiler_total_saving = 0;
                if (value.Length > 0 && value3.Length > 0)
                {
                    Total_Energy_pre = Program.UTIL.ToDoubleOrZero(value3[0][0]);
                    boiler_total_saving = Math.Max(0, Program.UTIL.ToDoubleOrZero(value3[0][0]) - Program.UTIL.ToDoubleOrZero(value[0][0]));
                }

                value = Program.DB.querySQL(DB.type.ProjDB, "Select 총에너지소요량 From FinalEnergy_Result_Rule Where 월='연간' and 연료='전기' and 검토유형='보일러'");
                value3 = Program.DB.getValue(DB.type.ProjDB, "FinalEnergy_Result", "총에너지소요량", "월='연간' and 연료='전기'");
                double boiler_total_elec = 0;
                if (value.Length > 0 && value3.Length > 0)
                {
                    boiler_total_elec = Math.Max(0, Program.UTIL.ToDoubleOrZero(value3[0][0]) - Program.UTIL.ToDoubleOrZero(value[0][0]));
                }
                value = Program.DB.querySQL(DB.type.ProjDB, "Select 총에너지소요량 From FinalEnergy_Result_Rule Where 월='연간' and 연료='전체' and 검토유형='보일러'");
                value3 = Program.DB.getValue(DB.type.ProjDB, "FinalEnergy_Result", "총에너지소요량", "월='연간' and 연료='전체'");
                double boiler_total_gas = 0;
                if (value.Length > 0 && value3.Length > 0)
                {
                    boiler_total_gas = Math.Max(0, Program.UTIL.ToDoubleOrZero(value3[0][0]) - Program.UTIL.ToDoubleOrZero(value[0][0]));
                }
                ...
                double[] Boiler_Power = new double[8]; double[] Boiler_eta_New = new double[8]; double[] Boiler_Point = new double[8]; double[] Boiler_eta_Rule = new double[8];
                ...
                if (Value.Length > 0)
                {
                    for (int a = 0; a < Value.Length; a++)
                    {
                        ...
                        Boiler_Power[a] = Program.UTIL.ToDoubleOrZero(Value[a][3]) * Program.UTIL.ToDoubleOrZero(Value[a][8]);
                        Boiler_eta_New[a] = Program.UTIL.ToDoubleOrZero(Value[a][4]);
                        Boiler_eta_Rule[a] = 90;//EPI 1점 
                ...
                if (Value.Length > 0)
                {
                    for (int a = 0; a < Value.Length; a++)
                    {
                        ...
                        Boiler_Power[a + Boiler_HW_count] = Program.UTIL.ToDoubleOrZero(Value[a][3]) * Program.UTIL.ToDoubleOrZero(Value[a][8]);
                        Boiler_eta_New[a + Boiler_HW_count] = Program.UTIL.ToDoubleOrZero(Value[a][4]);
                        Boiler_eta_Rule[a + Boiler_HW_count] = 90; //EPI 1점
                ...
                if (Value.Length > 0)
                {
                    for (int a = 0; a < Value.Length; a++)
                    {
                        ...
                        Boiler_Power[a + Boiler_HW_count + Boiler_H_count] = Program.UTIL.ToDoubleOrZero(Value[a][3]) * Program.UTIL.ToDoubleOrZero(Value[a][8]);
                        Boiler_eta_New[a + Boiler_HW_count + Boiler_H_count] = Program.UTIL.ToDoubleOrZero(Value[a][4]);
                        Boiler_eta_Rule[a + Boiler_HW_count + Boiler_H_count] = 90; //EPI 1점 
                ...
                double Boiler_eta_New_total = 0; double Boiler_Point_total = 0; double Boiler_eta_Rule_total = 0;
                for (int a = 0; a < 8; a++)
                {
                    ...
                    Boiler_eta_Rule_total += Boiler_eta_Rule[a] * Boiler_Power[a];
                }
                if (Boiler_Power.Sum() > 0)
                {
                    ...
                    Boiler_eta_Rule_total = Boiler_eta_Rule_total / Boiler_Power.Sum();
                }


                double v = double.IsNaN(boiler_total_saving) ? 0 : boiler_total_saving;
                Boiler_data[64].Add(new { idx = i, val = v.ToString("#,##0") });//절감량 전체 
                data.Add(new { cname = "boiler_saving_total", data = Boiler_data[64] });

                v = double.IsNaN(boiler_total_saving / Total_Energy_pre * 100) ? 0 : boiler_total_saving / Total_Energy_pre * 100;
                Boiler_data[65].Add(new { idx = i, val = v.ToString("0.0") + " %" });//절감률 전체 
                data.Add(new { cname = "boiler_saving_percent", data = Boiler_data[65] });

                v = double.IsNaN(boiler_total_elec * 0.4747 / 1000000 * 1000 + boiler_total_gas / 38.9 / 0.277778 * 38.5 * 15.236 / 1000000 * 44 / 12 * 1000 / 1000) ? 0 : boiler_total_elec * 0.4747 / 1000000 * 1000 + boiler_total_gas / 38.9 / 0.277778 * 38.5 * 15.236 / 1000000 * 44 / 12 * 1000 / 1000;
                Boiler_data[66].Add(new { idx = i, val = v.ToString("0.0") });//tco2
                data.Add(new { cname = "boiler_tco2", data = Boiler_data[66] });

                v = double.IsNaN(boiler_total_elec * 0.00023 + boiler_total_gas / 38.9 / 0.277778 * 0.00103) ? 0 : boiler_total_elec * 0.00023 + boiler_total_gas / 38.9 / 0.277778 * 0.00103;
                Boiler_data[67].Add(new { idx = i, val = v.ToString("0.0") });//절감량 전체 
                ...
                for (int a = 0; a < Boiler_Zones_split_H_W.Count; a++)
                {
                    ...
                    {
                        Boiler_ZoneArea += Program.UTIL.ToDoubleOrZero(ZoneValue[0][0]);
                    }
                }
                for (int a = 0; a < Boiler_Zones_split_H.Count; a++)
                {
                    string[][] ZoneValue = Program.DB.getValue(DB.type.ProjDB, "Zone_52016_Result", "Q_max", "존번호='" + Boiler_Zones_split_H[a].ToString() + "' And 난방_냉방='난방' and 월='1월'");
                    if (ZoneValue.Length > 0 && ZoneValue[0][0] != "")
                    {
                        Boiler_Qmax_h += Program.UTIL.ToDoubleOrZero(ZoneValue[0][0]);
                    }
                }
                for (int a = 0; a < Boiler_Zones_split_W.Count; a++)
                {
                    ...
                    if (ZoneValue.Length > 0 && ZoneValue[0][0] != "" && ZoneValue[0][1] != "")
                    {
                        ...
                        if (Usage.Length > 0)
                        { Boiler_Qmax_w += Program.UTIL.ToDoubleOrZero(ZoneValue[0][0]) * Program.UTIL.ToDoubleOrZero(Usage[0][0]); }
                    }
                }
                ...
                if (Boiler_eta_Rule_total > 0)
                {
                    Boiler_data[77].Add(new { idx = i, val = "* 효율 : " + Boiler_eta_Rule_total.ToString("0.0") + " % 기준 [EPI 1점]" });//난방 법규 평균  
                    data.Add(new { cname = " boiler_eta_rule", data = Boiler_data[77] });
                }
                #endregion
                ...
                Total_Energy_pre = 0;
                value = Program.DB.querySQL(DB.type.ProjDB, "Select 총에너지소요량 From FinalEnergy_Result_Rule Where 월='연간' and 연료='전체' and 검토유형='태양열'");
                value3 = Program.DB.getValue(DB.type.ProjDB, "FinalEnergy_Result", "총에너지소요량", "월='연간' and 연료='전체'");
                double solar_total_saving = 0;
                if (value.Length > 0 && value3.Length > 0)
                {
                    Total_Energy_pre = Program.UTIL.ToDoubleOrZero(value3[0][0]);
                    solar_total_saving = Math.Max(0, Program.UTIL.ToDoubleOrZero(value3[0][0]) - Program.UTIL.ToDoubleOrZero(value[0][0]));
                }

                value = Program.DB.querySQL(DB.type.ProjDB, "Select 총에너지소요량 From FinalEnergy_Result_Rule Where 월='연간' and 연료='전기' and 검토유형='태양열'");
                value3 = Program.DB.getValue(DB.type.ProjDB, "FinalEnergy_Result", "총에너지소요량", "월='연간' and 연료='전기'");
                double solar_total_elec = 0;
                if (value.Length > 0 && value3.Length > 0)
                {
                    solar_total_elec = Math.Max(0, Program.UTIL.ToDoubleOrZero(value3[0][0]) - Program.UTIL.ToDoubleOrZero(value[0][0]));
                }
                value = Program.DB.querySQL(DB.type.ProjDB, "Select 총에너지소요량 From FinalEnergy_Result_Rule Where 월='연간' and 연료='전체' and 검토유형='태양열'");
                value3 = Program.DB.getValue(DB.type.ProjDB, "FinalEnergy_Result", "총에너지소요량", "월='연간' and 연료='전체'");
                double solar_total_gas = 0;
                if (value.Length > 0 && value3.Length > 0)
                {
                    solar_total_gas = Math.Max(0, Program.UTIL.ToDoubleOrZero(value3[0][0]) - Program.UTIL.ToDoubleOrZero(value[0][0]));
                }
                ...
                double[] Solar_marea_new = new double[8]; double[] Solar_eta = new double[8]; double[] Solar_Point = new double[8]; double[] Solar_eta_Rule = new double[8];
                ...
                if (Value.Length > 0)
                {
                    for (int a = 0; a < Value.Length; a++)
                    {
                        ...
                        Solar_marea_new[a] = Program.UTIL.ToDoubleOrZero(Value[a][3]) * Program.UTIL.ToDoubleOrZero(Value[a][7]);
                        Solar_eta[a] = Program.UTIL.ToDoubleOrZero(Value[a][4]);
                        Solar_eta_Rule[a] = 0.71; //DIN V 18599-8 table 19
                ...
                if (Value.Length > 0)
                {
                    for (int a = 0; a < Value.Length; a++)
                    {
                        ...
                        Solar_marea_new[a + Solar_HW_count] = Program.UTIL.ToDoubleOrZero(Value[a][3]) * Program.UTIL.ToDoubleOrZero(Value[a][7]);
                        Solar_eta[a + Solar_HW_count] = Program.UTIL.ToDoubleOrZero(Value[a][4]);
                ...
                if (Value.Length > 0)
                {
                    for (int a = 0; a < Value.Length; a++)
                    {
                        ...
                        Solar_marea_new[a + Solar_HW_count + Solar_H_count] = Program.UTIL.ToDoubleOrZero(Value[a][3]) * Program.UTIL.ToDoubleOrZero(Value[a][7]);
                        Solar_eta[a + Solar_HW_count + Solar_H_count] = Program.UTIL.ToDoubleOrZero(Value[a][4]);
                ...
                double Solar_eta_total = 0; double Solar_Point_total = 0;
                for (int a = 0; a < 8; a++)
                {
                    ...
                    //가중평균 
                    Solar_eta_total += Solar_eta[a] * Solar_marea_new[a];
                    Solar_Point_total += Solar_Point[a] * Solar_marea_new[a];
                }
                if (Solar_marea_new.Sum() > 0)
                {
                    Solar_eta_total = Solar_eta_total / Solar_marea_new.Sum();
                    Solar_Point_total = Math.Min(100, Solar_Point_total / Solar_marea_new.Sum());
                }

                double v2 = double.IsNaN(solar_total_saving) ? 0 : solar_total_saving;
                Solar_data[64].Add(new { idx = i, val =v2.ToString("#,##0") });//절감량 전체 
                data.Add(new { cname = "solar_saving_total", data = Solar_data[64] });

                v2 = double.IsNaN(solar_total_saving / Total_Energy_pre * 100) ? 0 : solar_total_saving / Total_Energy_pre * 100;
                Solar_data[65].Add(new { idx = i, val = v2.ToString("0.0") + " %" });//절감률 전체 
                data.Add(new { cname = "solar_saving_percent", data = Solar_data[65] });

                v2 = double.IsNaN(solar_total_elec * 0.4747 / 1000000 * 1000 + solar_total_gas / 38.9 / 0.277778 * 38.5 * 15.236 / 1000000 * 44 / 12 * 1000 / 1000) ? 0 : solar_total_elec * 0.4747 / 1000000 * 1000 + solar_total_gas / 38.9 / 0.277778 * 38.5 * 15.236 / 1000000 * 44 / 12 * 1000 / 1000;
                Solar_data[66].Add(new { idx = i, val = v2.ToString("0.0") });//tco2
                data.Add(new { cname = "solar_tco2", data = Solar_data[66] });

                v2 = double.IsNaN(solar_total_elec * 0.00023 + solar_total_gas / 38.9 / 0.277778 * 0.00103) ? 0 : solar_total_elec * 0.00023 + solar_total_gas / 38.9 / 0.277778 * 0.00103;
                Solar_data[67].Add(new { idx = i, val = v2.ToString("0.0") });//절감량 전체 
                ...
                data.Add(new { cname = "solar_marea_new_total", data = Solar_data[68] });
                Solar_data[70].Add(new { idx = i, val = Solar_eta_total.ToString("0.0") });//효율 평균  
                data.Add(new { cname = "solar_eta_total", data = Solar_data[70] });
                d = Solar_Point_total;
                if (d >= 100) { sp = "<div class='cls-sparkline-blue' style='width:" + (int)((d * 117) / 100) + "px'></div>"; }//성능 수준 중 가장 큰 값을을 100로 가정, 117는 픽셀 최대 크기
                else if (d <= 30) { sp = "<div class='cls-sparkline-red' style='width:" + (int)((d * 117) / 100) + "px'></div>"; }//성능 수준 중 가장 큰 값을을 100로 가정, 117는 픽셀 최대 크기
                else { sp = "<div class='cls-sparkline' style='width:" + (int)((d * 117) / 100) + "px'></div>"; }
                ...
                for (int a = 0; a < Solar_Zones_split_H_W.Count; a++)
                {
                    ...
                    if (ZoneValue.Length > 0 && ZoneValue[0][0] != "")
                    {
                        Solar_ZoneArea += Program.UTIL.ToDoubleOrZero(ZoneValue[0][0]);
                    }
                }
                for (int a = 0; a < Solar_Zones_split_H.Count; a++)
                {
                    string[][] ZoneValue = Program.DB.getValue(DB.type.ProjDB, "Zone_52016_Result", "Q_max", "존번호='" + Solar_Zones_split_H[a].ToString() + "' And 난방_냉방='난방' and 월='1월'");
                    if (ZoneValue.Length > 0 && ZoneValue[0][0] != "")
                    {
                        Solar_Qmax_h += Program.UTIL.ToDoubleOrZero(ZoneValue[0][0]);
                    }
                }
                for (int a = 0; a < Solar_Zones_split_W.Count; a++)
                {
                    ...
                    if (ZoneValue.Length > 0 && ZoneValue[0][0] != "" && ZoneValue[0][1] != "")
                    {
                        string[][] Usage = Program.DB.getValue(DB.type.BaseDB_HCneed, "용도프로필", "급탕시간당비율", "용도명 = '" + ZoneValue[0][1] + "'");
                        if (Usage.Length > 0)
                        { Solar_Qmax_w += Program.UTIL.ToDoubleOrZero(ZoneValue[0][0]) * Program.UTIL.ToDoubleOrZero(Usage[0][0]); }
                    }
                }
                ...
                if (Solar_marea_new.Sum() > 0)
                {
                    Solar_data[77].Add(new { idx = i, val = "* DIN V 18599-8 표준 효율 : " + Solar_eta_Rule[0].ToString("0.0") + " 기준" });//난방 법규 평균  
                    data.Add(new { cname = " solar_eta_rule", data = Solar_data[77] });
                }
                #endregion

                #region 급탕히트펌프 
                Total_Energy_pre = 0;
                value = Program.DB.querySQL(DB.type.ProjDB, "Select 총에너지소요량 From FinalEnergy_Result_Rule Where 월='연간' and 연료='전체' and 검토유형='급탕HP'");
                value3 = Program.DB.getValue(DB.type.ProjDB, "FinalEnergy_Result", "총에너지소요량", "월='연간' and 연료='전체'");
                double WHP_total_saving = 0;
                if (value.Length > 0 && value3.Length > 0)
                {
                    Total_Energy_pre = Program.UTIL.ToDoubleOrZero(value3[0][0]);
                    WHP_total_saving = Math.Max(0, Program.UTIL.ToDoubleOrZero(value3[0][0]) - Program.UTIL.ToDoubleOrZero(value[0][0]));
                }

                value = Program.DB.querySQL(DB.type.ProjDB, "Select 총에너지소요량 From FinalEnergy_Result_Rule Where 월='연간' and 연료='전기' and 검토유형='급탕HP'");
                value3 = Program.DB.getValue(DB.type.ProjDB, "FinalEnergy_Result", "총에너지소요량", "월='연간' and 연료='전기'");
                double WHP_total_elec = 0;
                if (value.Length > 0 && value3.Length > 0)
                {
                    WHP_total_elec = Math.Max(0, Program.UTIL.ToDoubleOrZero(value3[0][0]) - Program.UTIL.ToDoubleOrZero(value[0][0]));
                }
                value = Program.DB.querySQL(DB.type.ProjDB, "Select 총에너지소요량 From FinalEnergy_Result_Rule Where 월='연간' and 연료='전체' and 검토유형='급탕HP'");
                value3 = Program.DB.getValue(DB.type.ProjDB, "FinalEnergy_Result", "총에너지소요량", "월='연간' and 연료='전체'");
                double WHP_total_gas = 0;
                if (value.Length > 0 && value3.Length > 0)
                {
                    WHP_total_gas = Math.Max(0, Program.UTIL.ToDoubleOrZero(value3[0][0]) - Program.UTIL.ToDoubleOrZero(value[0][0]));
                }
                string[] WHP_Name = new string[8]; string[] WHP_Zone_text = new string[8];
                double[] WHP_Power = new double[8]; double[] WHP_COP_New = new double[8]; double[] WHP_Point = new double[8]; double[] WHP_COP_Rule = new double[8];
                ...
                Value = Program.DB.querySQL(DB.type.ProjDB, "Select b.히트펌프번호,a.명칭,b.존,a.급탕정격용량,a.급탕정격COP,a.난방급탕,b.번호,b.태양열번호,b.히트펌프대수 From User_DHWHP as a Inner Join DHWSystem_Form as b ON a.번호 = b.히트펌프번호 Where a.난방급탕='급탕'");
                ...
                if (Value.Length > 0)
                {
                    for (int a = 0; a < Value.Length; a++)
                    {
                        ...
                        if (Value[a][8] != "")
                        {
                            WHP_Power[a] = Program.UTIL.ToDoubleOrZero(Value[a][3]) * Program.UTIL.ToDoubleOrZero(Value[a][8]);
                            WHP_COP_New[a] = Program.UTIL.ToDoubleOrZero(Value[a][4]);


                            WHP_COP_Rule[a] = 3.06; // DIN V 18599-8 table b.4
                            WHP_Point[a] = Math.Min(100, WHP_COP_New[a] / WHP_COP_Rule[a] * 100);
                        }

                    }
                }
                double WHP_COP_New_total = 0; double WHP_Point_total = 0; double WHP_COP_Rule_total = 0;
                for (int a = 0; a < 8; a++)
                {
                    ...
                    //가중평균 
                    WHP_COP_New_total += WHP_COP_New[a] * WHP_Power[a];
                    WHP_Point_total += WHP_Point[a] * WHP_Power[a];
                    WHP_COP_Rule_total += WHP_COP_Rule[a] * WHP_Power[a];
                }
                if (WHP_Power.Sum() > 0)
                {
                    WHP_COP_New_total = WHP_COP_New_total / WHP_Power.Sum();
                    WHP_Point_total = WHP_Point_total / WHP_Power.Sum();
                    WHP_COP_Rule_total = WHP_COP_Rule_total / WHP_Power.Sum();
                }

                double v3 = double.IsNaN(WHP_total_saving) ? 0 : WHP_total_saving;
               WHP_data[64].Add(new { idx = i, val = v3.ToString("#,##0") });//절감량 전체 
                data.Add(new { cname = "whp_saving_total", data = WHP_data[64] });

                v3 = double.IsNaN(WHP_total_saving / Total_Energy_pre * 100) ? 0 : WHP_total_saving / Total_Energy_pre * 100;
                WHP_data[65].Add(new { idx = i, val = v3.ToString("0.0") + " %" });//절감률 전체 
                data.Add(new { cname = "whp_saving_percent", data = WHP_data[65] });

                v3 = double.IsNaN(WHP_total_elec * 0.4747 / 1000000 * 1000 + WHP_total_gas / 38.9 / 0.277778 * 38.5 * 15.236 / 1000000 * 44 / 12 * 1000 / 1000) ? 0 : WHP_total_elec * 0.4747 / 1000000 * 1000 + WHP_total_gas / 38.9 / 0.277778 * 38.5 * 15.236 / 1000000 * 44 / 12 * 1000 / 1000;
                WHP_data[66].Add(new { idx = i, val = v3.ToString("0.0") });//tco2
                data.Add(new { cname = "whp_tco2", data = WHP_data[66] });

                v3 = double.IsNaN(WHP_total_elec * 0.00023 + WHP_total_gas / 38.9 / 0.277778 * 0.00103) ? 0 : WHP_total_elec * 0.00023 + WHP_total_gas / 38.9 / 0.277778 * 0.00103;
                WHP_data[67].Add(new { idx = i, val = v3.ToString("0.0") });//절감량 전체 
                ...
                WHP_data[70].Add(new { idx = i, val = WHP_COP_New_total.ToString("0.0") });//효율 평균  
                data.Add(new { cname = "whp_cop_new_total", data = WHP_data[70] });
                ...
                for (int a = 0; a < WHP_Zones_split_H_W.Count; a++)
                {
                    ...
                    if (ZoneValue.Length > 0 && ZoneValue[0][0] != "")
                    {
                        WHP_ZoneArea += Program.UTIL.ToDoubleOrZero(ZoneValue[0][0]);
                    }
                }
                for (int a = 0; a < WHP_Zones_split_H.Count; a++)
                {
                    string[][] ZoneValue = Program.DB.getValue(DB.type.ProjDB, "Zone_52016_Result", "Q_max", "존번호='" + WHP_Zones_split_H[a].ToString() + "' And 난방_냉방='난방' and 월='1월'");
                    if (ZoneValue.Length > 0 && ZoneValue[0][0] != "")
                    {
                        WHP_Qmax_h += Program.UTIL.ToDoubleOrZero(ZoneValue[0][0]);
                    }
                }
                for (int a = 0; a < WHP_Zones_split_W.Count; a++)
                {
                    ...
                    if (ZoneValue.Length > 0 && ZoneValue[0][0] != "" && ZoneValue[0][1] != "")
                    {
                        string[][] Usage = Program.DB.getValue(DB.type.BaseDB_HCneed, "용도프로필", "급탕시간당비율", "용도명 = '" + ZoneValue[0][1] + "'");
                        if (Usage.Length > 0)
                        { WHP_Qmax_w += Program.UTIL.ToDoubleOrZero(ZoneValue[0][0]) * Program.UTIL.ToDoubleOrZero(Usage[0][0]); }
                    }
                }
        ...
        public string Report_After()
        {
            ...
            while (++i < 번호.Length)
            {
                if (res.Length > 0)
                {
                                ...
                                if (Value[a][6] == hh.Num_New())
                                {
                                    ...
                                    for (int aaa = 0; aaa < hh.Num_Old().Count; aaa++)
                                    {
                                        ...
                                        if (OldSystem.Length > 0)
                                        {
                                            ...
                                            { Boiler_eta_Old[a] = Program.UTIL.ToDoubleOrZero(Value[a][4]); break; }
                                            else if (Program.UTIL.ToDoubleOrZero(OldSystem[0][0]) < Boiler_eta_Old[a]) { Boiler_eta_Old[a] = Program.UTIL.ToDoubleOrZero(OldSystem[0][0]); }
                                            else if (Boiler_eta_Old[a] == 0) { Boiler_eta_Old[a] = Program.UTIL.ToDoubleOrZero(OldSystem[0][0]); }
                                        }
                                    }
                                    if (Value[a][7] != "")
                                    {
                            ...
                            for (int aa = 0; aa < HeatingGroup.Count; aa++)
                            {
                                ...
                                if (Value[a][6] == hh.Num_New())
                                {
                                    ...
                                    for (int aaa = 0; aaa < hh.Num_Old().Count; aaa++)
                                    {
                                        ...
                                        {
                                            if (Program.UTIL.ToDoubleOrZero(OldSystem[0][0]) == Program.UTIL.ToDoubleOrZero(Value[a][4]))
                                            { Boiler_eta_Old[a + Boiler_HW_count] = Program.UTIL.ToDoubleOrZero(Value[a][4]); break; }
                                            else if (Program.UTIL.ToDoubleOrZero(OldSystem[0][0]) < Boiler_eta_Old[a + Boiler_HW_count]) { Boiler_eta_Old[a + Boiler_HW_count] = Program.UTIL.ToDoubleOrZero(OldSystem[0][0]); }
                                            else if (Boiler_eta_Old[a + Boiler_HW_count] == 0) { Boiler_eta_Old[a + Boiler_HW_count] = Program.UTIL.ToDoubleOrZero(OldSystem[0][0]); }
                                        }
                                    }
                                    if (Value[a][7] != "")
                                    {
                            ...
                            for (int aa = 0; aa < DHWGroup.Count; aa++)
                            {
                                ...
                                if (Value[a][6] == ww.Num_New())
                                {
                                    ...
                                    for (int aaa = 0; aaa < ww.Num_Old().Count; aaa++)
                                    {
                                        ...
                                        {
                                            if (Program.UTIL.ToDoubleOrZero(OldSystem[0][0]) == Program.UTIL.ToDoubleOrZero(Value[a][4]))
                                            { Boiler_eta_Old[a + Boiler_HW_count + Boiler_H_count] = Program.UTIL.ToDoubleOrZero(Value[a][4]); break; }
                                            else if (Program.UTIL.ToDoubleOrZero(OldSystem[0][0]) < Boiler_eta_Old[a + Boiler_HW_count + Boiler_H_count]) { Boiler_eta_Old[a + Boiler_HW_count + Boiler_H_count] = Program.UTIL.ToDoubleOrZero(OldSystem[0][0]); }
                                            else if (Boiler_eta_Old[a + Boiler_HW_count + Boiler_H_count] == 0) { Boiler_eta_Old[a + Boiler_HW_count + Boiler_H_count] = Program.UTIL.ToDoubleOrZero(OldSystem[0][0]); }
                                        }
                                    }
                                    if (Value[a][7] != "")
                                    {
                    ...
                    double Boiler_eta_New_total = 0; double Boiler_eta_Old_total = 0; double Boiler_Point_total = 0; double Boiler_eta_Rule_total = 0;
                    for (int a = 0; a < 8; a++)
                    {
                        Boiler_data[a].Add(new { idx = i, val = Boiler_Name[a] });//명칭
                        data.Add(new { cname = "boiler_name" + a, data = Boiler_data[a] });
                            ...
                            for (int aa = 0; aa < DHWGroup.Count; aa++)
                            {
                                ...
                                if (Value[a][6] == cc.Num_New())
                                {
                                    ...
                                    for (int aaa = 0; aaa < cc.Num_Old().Count; aaa++)
                                    {
                                        ...
                                        if (OldSystem.Length > 0)
                                        {
                                            Solar_marea_Old[a] = Program.UTIL.ToDoubleOrZero(OldSystem[0][0]) * Program.UTIL.ToDoubleOrZero(OldSystem[0][1]);
                                        }
                                    }
                                }
                            }
                            ...
                            for (int aa = 0; aa < HeatingGroup.Count; aa++)
                            {
                                ...
                                if (Value[a][6] == hh.Num_New())
                                {
                                    ...
                                    for (int aaa = 0; aaa < hh.Num_Old().Count; aaa++)
                                    {
                                        ...
                                        if (OldSystem.Length > 0)
                                        {
                                            Solar_marea_Old[a + Solar_HW_count] = Program.UTIL.ToDoubleOrZero(OldSystem[0][0]) * Program.UTIL.ToDoubleOrZero(OldSystem[0][1]);
                                        }
                                    }
                                }
                            }
                            ...
                            for (int aa = 0; aa < DHWGroup.Count; aa++)
                            {
                                ...
                                if (Value[a][6] == ww.Num_New())
                                {
                                    ...
                                    for (int aaa = 0; aaa < ww.Num_Old().Count; aaa++)
                                    {
                                        ...
                                        if (OldSystem.Length > 0)
                                        {
                                            Solar_marea_Old[a + Solar_HW_count + Solar_H_count] = Program.UTIL.ToDoubleOrZero(OldSystem[0][0]) * Program.UTIL.ToDoubleOrZero(OldSystem[0][1]);
                                        }
                                    }
                                }
                            }
                    ...
                    double Solar_eta_total = 0; double Solar_marea_old_total = 0; double Solar_Point_total = 0;
                    for (int a = 0; a < 8; a++)
                    {
                        Solar_data[a].Add(new { idx = i, val = Solar_Name[a] });//명칭
                        data.Add(new { cname = "solar_name" + a, data = Solar_data[a] });
                        ...
                        //가중평균 
                        Solar_eta_total += Solar_eta[a] * Solar_marea_new[a];
                        Solar_marea_old_total += Solar_marea_Old[a] * Solar_marea_new[a];
                        Solar_Point_total += Solar_Point[a] * Solar_marea_new[a];
                    }
                    if (Solar_marea_new.Sum() > 0)
                    {
                        Solar_eta_total = Solar_eta_total / Solar_marea_new.Sum();
                        Solar_marea_old_total = Solar_marea_old_total / Solar_marea_new.Sum();
                        Solar_Point_total = Math.Min(100, Solar_Point_total / Solar_marea_new.Sum());
                    }
                    ...
                    Solar_data[68].Add(new { idx = i, val = Solar_marea_new.Sum().ToString("0.0") });//용량 합계  
                    data.Add(new { cname = "solar_marea_new_total", data = Solar_data[68] });
                    Solar_data[69].Add(new { idx = i, val = Solar_marea_old_total.ToString("0.0") });//기존 효율 평균  
                    data.Add(new { cname = "solar_marea_old_total", data = Solar_data[69] });
                    Solar_data[70].Add(new { idx = i, val = Solar_eta_total.ToString("0.0") });//효율 평균  
                    data.Add(new { cname = "solar_eta_total", data = Solar_data[70] });
                    ...
                    #region 급탕히트펌프 
                    string[] WHP_Name = new string[8]; string[] WHP_Zone_text = new string[8];
                    double[] WHP_Power = new double[8]; double[] WHP_COP_Old = new double[8]; double[] WHP_COP_New = new double[8]; double[] WHP_Saving = new double[8]; double[] WHP_Point = new double[8]; double[] WHP_COP_Rule = new double[8];
                    string[] WHP_H_W = new string[8];
                            ...
                            for (int aa = 0; aa < DHWGroup.Count; aa++)
                            {
                                ...
                                if (Value[a][6] == ww.Num_New())
                                {
                                    ...
                                    for (int aaa = 0; aaa < ww.Num_Old().Count; aaa++)
                                    {
                                        ...
                                        {
                                            if (Program.UTIL.ToDoubleOrZero(OldSystem[0][0]) == Program.UTIL.ToDoubleOrZero(Value[a][4]))
                                            { WHP_COP_Old[a] = Program.UTIL.ToDoubleOrZero(Value[a][4]); break; }
                                            else if (Program.UTIL.ToDoubleOrZero(OldSystem[0][0]) < WHP_COP_Old[a]) { WHP_COP_Old[a] = Program.UTIL.ToDoubleOrZero(OldSystem[0][0]); }
                                            else if (WHP_COP_Old[a] == 0) { WHP_COP_Old[a] = Program.UTIL.ToDoubleOrZero(OldSystem[0][0]); }
                                        }
                                    }
                                    if (Value[a][7] != "")
                                    {
                    ...
                    double WHP_COP_New_total = 0; double WHP_COP_Old_total = 0; double WHP_Point_total = 0; double WHP_COP_Rule_total = 0;
                    for (int a = 0; a < 8; a++)
                    {
                        WHP_data[a].Add(new { idx = i, val = WHP_Name[a] });//명칭
                        data.Add(new { cname = "whp_name" + a, data = WHP_data[a] });
                        ...
                        //가중평균 
                        WHP_COP_New_total += WHP_COP_New[a] * WHP_Power[a];
                        WHP_COP_Old_total += WHP_COP_Old[a] * WHP_Power[a];
                        WHP_Point_total += WHP_Point[a] * WHP_Power[a];
                        WHP_COP_Rule_total += WHP_COP_Rule[a] * WHP_Power[a];
                    }
                    if (WHP_Power.Sum() > 0)
                    {
                        WHP_COP_New_total = WHP_COP_New_total / WHP_Power.Sum();
                        WHP_COP_Old_total = WHP_COP_Old_total / WHP_Power.Sum();
                        WHP_Point_total = WHP_Point_total / WHP_Power.Sum();
                        WHP_COP_Rule_total = WHP_COP_Rule_total / WHP_Power.Sum();
                    }
                    ...
                    WHP_data[68].Add(new { idx = i, val = WHP_Power.Sum().ToString("0.0") });//용량 합계  
                    data.Add(new { cname = "whp_power_total", data = WHP_data[68] });
                    WHP_data[69].Add(new { idx = i, val = WHP_COP_Old_total.ToString("0.0") });//기존 효율 평균  
                    data.Add(new { cname = "whp_cop_old_total", data = WHP_data[69] });
                    WHP_data[70].Add(new { idx = i, val = WHP_COP_New_total.ToString("0.0") });//효율 평균  
                    data.Add(new { cname = "whp_cop_new_total", data = WHP_data[70] });
                    WHP_data[71].Add(new { idx = i, val = (WHP_total_saving / Total_Energy_pre * 100).ToString("0.0") + " %" });//절감량 합계  
                    data.Add(new { cname = "whp_saving_total2", data = WHP_data[71] });
