...
{
    public partial class ConstructionFloor : Form, IConfirmable
    {
        ...
        private void OldFloor_comboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (item != null)
            {
                ...
                {
                    OldFloor_R = 1 / Program.UTIL.ToDoubleOrZero(OldWall_U[0][0]);
                    Add_OldFloor();
                }
            }
            else { }
        ...
        public bool ValidateAndSave(bool isManualSave = false)
        {
            try
            {
                ...
                if (Type != "기존바닥" && Type != "내부덧댐" && Base != "지면위" && Base != "단열지하실" && TBName == null)
                {
                    missing.Add("열교");
                }
