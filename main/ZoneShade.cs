...
namespace main
{
    internal class ZoneShade
    {
        ...
        public ZoneShade() // 생성자 안에 trycatch
        {
            ...
            try
            {
                string[][] rec = Program.DB.getValue(DB.type.ProjDB, "ZoneEnvelope_3D", "존,우측면돌출각도,좌측면돌출각도,상부돌출각도,주변요소음영각도,우측면돌출길이,좌측면돌출길이,상부돌출길이,주변요소음영길이,번호,방위,기울기,외피유형,구조체번호,가로길이,세로길이", "아이디 = '" + ID + "'");   //창호 혹은 커튼월 가로 세로 나와야함 (임시로 가로길이, 세로길이라고함)
                rec[0][1] = 우측돌출부각도.ToString();
                rec[0][2] = 좌측돌출부각도.ToString();
                rec[0][3] = 상부돌출부각도.ToString();
                rec[0][4] = 주변지형물각도.ToString();
                rec[0][5] = 우측돌출부길이.ToString();
                rec[0][6] = 좌측돌출부길이.ToString();
                rec[0][7] = 상부돌출부길이.ToString();
                rec[0][8] = 주변지형물높이.ToString();
                rec[0][10] = 방위.ToString();
                rec[0][11] = 경사.ToString();
                ...
                for (int i = 0; i < 12; i++)
                {
                    string[][] aa = Program.DB.getValue(DB.type.BaseDB_HCneed, "기후데이터_방위각", "방위각", "지역명= '" + ValueA[0][0] + "' AND 방향 = '" + rec[0][10] + "' AND 각도 ='" + rec[0][11] + "˚" + "' AND 종류 = '" + "우측" + "' AND 기간 ='" + (i + 1).ToString() + "월" + "'");
