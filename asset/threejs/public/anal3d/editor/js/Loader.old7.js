...
import { InWalling } from './InWalling.js';
...
//			console.log( 'Loading', filename, size, progress );
		...
		switch ( extension ) {
			...
			{
				var material = new THREE.MeshStandardMaterial({color: 0xffffff, roughness: .7, transparent: true, opacity: .5}); 
				...
				reader.addEventListener( 'load', async function ( event ) {
					...
					let inwalling = new InWalling( editor );
					let windowing = new Windowing( editor );
					let spacing = new Spacing( editor );
					let shadows = new Shadows( editor );
					let bridges = new Bridges( editor );
					
					const aabb = new Box3().setFromObject( object );
					const center = aabb.getCenter( new Vector3() );
					const offset = new Vector3();
			
					offset.x =  object.position.x - center.x;
					offset.y = 0;
					offset.z = object.position.z - center.z;

					zoning.initPositions();

					object.traverse( function( child ) {            
						if ( child instanceof THREE.Mesh ) {                 
							zoning.collectPositions(offset, child.geometry.getAttribute("position"), child.geometry.getAttribute("normal"));   
						}
					});  
					...
					object.traverse( function( child ) {            
						if (child instanceof THREE.LineSegments && _isFlat(child.geometry.boundingBox)) {
							windowing.collectWindows(offset, child.geometry.boundingBox);   
						}
					});  
					inwalling.collectEdgedWalls();

					zoning.buildWalls();
					editor.shrinkRooms();
				//	editor.connectAllWindows();

				//	editor.duplicateInwalls();
					...
					editor.createSpacesInfo();

					bridges.collectBridges();

					bridges.calcBridges();

					editor.sendWallData(); //temp arcookie
