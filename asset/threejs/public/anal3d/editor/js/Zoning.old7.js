...
//						if(current_stack.length % 2 == 0) continue
			...
			let _getCardiInfo = (vtx) => {
				...
				while(++i < vtx.length) {
					...
					if (area > 0) {
						...
						return {cardi:cardinal, slope:slope};
					}
				}
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			while(++j < _loops.length) {
				...
				let info = _getCardiInfo(vertices);

				if (info) {
					k = -1;
					while(++k < vertices.length) {
						vertices[k].slope = info.slope;
					}

					if (!this.editor.wall[info.cardi]) this.editor.wall[info.cardi] = {};

					if (info.cardi.indexOf('UP_') < 0 && info.cardi !== '') {

						this.editor.wall[info.cardi][this.editor.wnum] = {"vertices":vertices,'type':'INWALL'};

						this.editor.wnum++;

						let ccardi = this.util.counterCardi(info.cardi);

						let cslope = info.slope == 0 || info.slope == 180 ? info.slope - 180 : info.slope;
						let vertices2 = JSON.parse(JSON.stringify(vertices));

						k = -1;
						while(++k < vertices2.length) {
							vertices2[k].slope = cslope;
						}
	
						if (!this.editor.wall[ccardi]) this.editor.wall[ccardi] = {};

						this.editor.wall[ccardi][this.editor.wnum] = {"vertices":vertices2,'type':'INWALL', inwalled:{idx:this.editor.wnum - 1,cardi:info.cardi}};
						this.editor.wall[info.cardi][this.editor.wnum - 1].inwalled = {idx:this.editor.wnum,cardi:ccardi};

						this.editor.wnum++;
					}
					else {
						this.editor.wall[info.cardi][this.editor.wnum++] = {"vertices":vertices,'type':_getType(info.slope, info.cardi, !!(_getCenterY(vertices) < 0))};
					}
				}
...
*/
		...
		// 	let T = this.util.asTriangle(vtx.position);
			
		// 	T.getNormal(v);
		// 	T.getMidpoint(v2);

		// 	let L1 = new THREE.Line3(v2, v.clone().multiplyScalar(9999).multiply(v2));
		// 	let L2 = new THREE.Line3(v2, v.clone().multiplyScalar(-9999).multiply(v2));

		// 	for (const [cardi, value] of Object.entries(this.editor.wall)) {
		// 		for (const [j, el] of Object.entries(value)) {
		// 			T = this.util.asTriangle(el.vertices[0].position);
		// 			T.getPlane(PL);

		// 			if (PL.intersectsLine(L1) && PL.intersectsLine(L2)) return true;
		// 		}
		// 	}
		// 	return false;
		// };
		...
		// for (const [cardi, value] of Object.entries(this.editor.wall)) {
		// 	for (const [j, el] of Object.entries(value)) {
		// 		let ccardi = this.util.counterCardi(cardi);

		// 		if (el.type != 'INWALL' && cardi.indexOf('UP_') < 0 && ccardi !== '' && _isInWall(el.vertices[0])) {
		// 			let cslope = vtx.slope == 0 || vtx.slope == 180 ? vtx.slope - 180 : vtx.slope;
		// 			let vertices = JSON.parse(JSON.stringify(el.vertices));

		// 			el.type = 'INWALL';

		// 			k = -1;
		// 			while(++k < vertices.length) {
		// 				vertices[k].slope = cslope;
		// 			}

		// 			if (!this.editor.wall[ccardi]) this.editor.wall[ccardi] = {};
		// 			this.editor.wall[ccardi][this.editor.wnum++] = {"vertices":vertices,'type':'INWALL'};
		// 		}
		// 	}
		// }
