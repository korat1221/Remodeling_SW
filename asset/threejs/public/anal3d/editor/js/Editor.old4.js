...
import { IfcRelConnectsPortToElement } from '../../examples/jsm/loaders/ifc/web-ifc-api.js';
...
//						rects.push(new THREE.Vector3(-0.1,5,-0.02));
	...
	makeVerticesItems: function ( offset, position, normal ) {
		let i = 0, j, k, area, slope, pos, nom, cardinal;
		...
		while(i < position.length) {
			...
			if ((area = this.getArea(pos)) > 0) {
				...
				if (!this.walls[cardinal]) {
					this.walls[cardinal] = {};
				}
	
				var linked = [], n = 0;
	
				for (const [cardi, value] of Object.entries(this.walls)) {
					for (const [idx, el] of Object.entries(value)) {
						k = -1;
						while (++k < el.vertices.length) {
							let el2 = el.vertices[k];
	
							if ((n = this.getSamePoints(el2.position, pos).length) == 2) {
								if (cardinal == cardi && this.isGArea(el2.position) == this.isGArea(pos)) {
									if (linked.length == 0) {
										el.vertices.push({"position":pos,"slope":slope,"area":area});
									}
									linked.push({crd:cardi, idx:idx});		
								}
							}
							if(n >= 2)  {
								break;
				...
				if(n <= 2)  {
					if (linked.length == 0) {
						let idx = (this.wnum++);
						this.walls[cardinal][idx] = {"vertices":[{"position":pos,"slope":slope,"area":area}]};
					}
					else {
						if (linked.length > 1) {
							j = 0;
							...
							while(++j < linked.length) {
								this.walls[linked[0].crd][linked[0].idx].vertices = this.walls[linked[0].crd][linked[0].idx].vertices.concat(this.walls[linked[j].crd][linked[j].idx].vertices);
								delete this.walls[linked[j].crd][linked[j].idx];
							}
						}
					}
				}
	...
	normalizeVItems: function () {
		let ret = [], i = -1, j, k, l;
/*
		while(++i < vCont.vi.length) {
			let vi = vCont.vi[i];
			let cardi = this.getCounterCardi(vi.cardinal);
			if (cardi !== '' && vCont.walls[cardi]) {
				for (const [j, el] of Object.entries(vCont.walls[cardi])) {
					k = -1;
					while(++k < vi.pos.length) {
						let p = vi.pos[k];

						l = -1;
						while(++l < el.vertices.length) {
							let el3 = el.vertices[l];
							if (this.inTriangle(el3.position, p)) {
								if (!el.extra) el.extra = [];

								if (!el.extra.find(el4 => {
									return !!(el4[0] == p[0] && el4[1] == p[1] && el4[2] == p[2]);
								})) {
									el.extra.push(p);
								}
							}
...
*/
		...
		let _compare = (a, b) => {
			...
			while(++i < key.length) {
				if (_isSamePlane(a, b[key[i]]) && _inTriangle(a,b[key[i]])) {
					if (!_collects.find(el => { return !!(el.equals(b)); })) {
						_collects.push(b);
						return;
					}
				}
		...
		let _compares = (a, b) => {
			...
			while(++i < a.length) {
				j = -1;
				while(++j < b.length) {
					_compare(_asTriangle(a[i].position), _asTriangle(b[j].position));			
				}
			}
		};
		...
		for (const [cardi, val] of Object.entries(this.walls)) {
			for (const [idx, wall] of Object.entries(val)) {
				_collects = [];
				for (const [cardi2, val2] of Object.entries(this.walls)) {
					for (const [idx2, wall2] of Object.entries(val2)) {
						if ((cardi != cardi2 || idx != idx2)) _compares(wall.vertices, wall2.vertices);
					}
				}
				if (_collects.length > 0) {
					wall.extra = _collects;
				}
			}
		}
	...
	//	let slope, area, pos;
		...
		for (const [cardi, value] of Object.entries(this.walls)) {
			for (const [idx, el] of Object.entries(value)) {
				// if (el.extra && el.vertices.length > 0) {
				// 	let points = [];
				...
				// 		let el2 = el.vertices[k];
				// 		l = -1;
				// 		while(++l < el2.position.length) {
				// 			let el3 = el2.position[l];
				// 	//		if (!points.find(el4 => {
				// 	//			{ return !!(el4.x == el3[0] && el4.y == el3[1] && el4.z == el3[2]);}
				// 	//		})) {
				// 				points.push(new THREE.Vector3(el3[0], el3[1], el3[2]));
				// 	//		}
				// 		}
				// 	}

				// 	slope = el.vertices[0].slope;
					
				// //	let a = el.vertices[0].position[0];
				...
				while(++k < el.vertices.length) {
					let el2 = el.vertices[k];
					ret.push({cardinal:cardi,slope:el2.slope,area:el2.area,pos:el2.position});
					// if (this.debug.use) {
					// 	if (cardi == 'S') {
					// 		l = -1;
					// 		while(++l < el2.position.length) {
					// 			this.drawPoint(el2.position[l], 0x0000ff);
					// 			this.addDebugLine([el2.position[l], el2.position[(l + 1) % el2.position.length]])
					// 		}
					// 	}
					// }
				}
				...
				// 	if (el.extra) {
				// 		l = -1;
				// 		while(++l < el.extra.length) {
				// 			let a = el.extra[l];
				// 			this.drawPoint([a.a.x,a.a.y,a.a.z], 0xff0000);
				// //				this.addDebugLine([el2.position[l], el2.position[(l + 1) % el2.position.length]])
				// 		}
				// 	}
				// }
...
/*	
	...
	normalizeVItems: function ( vitems ) {
		let _edges = [], i = -1, j, k, _adding = [], _intersects = [];
		let _addIntersects = (_intersects, a, b, c) => {
			if (!_intersects.find(el => { return !!(el.key[0][0] == a[0][0] && el.key[0][1] == a[0][1] && el.key[0][2] == a[0][2] && el.key[1][0] == a[1][0] && el.key[1][1] == a[1][1] && el.key[1][2] == a[1][2]); })) {
				_intersects.push({key:a,val:[b,c]});
			}
		};
		let _subLines = (a, b) => {
			let c;
		...
		let _changeAllPoints = (_intersects, pos) => {
			if (pos.length > 0) {
				let i = -1;

				while(++i < pos.length) {
					let a = [pos[i],pos[(i+1) % pos.length]];
...
//						return;
				...
				while(++i < pos.length) {
					let a = [pos[i],pos[(i+1) % pos.length]];
					let b = _intersects.find(el => { return !!(el.key[0][0] == a[0][0] && el.key[0][1] == a[0][1] && el.key[0][2] == a[0][2] && el.key[1][0] == a[1][0] && el.key[1][1] == a[1][1] && el.key[1][2] == a[1][2]); });
					if (b) {
						a[0][0] = b.val[0][0];
						a[0][1] = b.val[0][1];
						a[0][2] = b.val[0][2];
		...
		let _addPos = (a,line) => {
			if (a[0][0] == line[0][0] && a[0][1] == line[0][1] && a[0][2] == line[0][2]) {
				return JSON.stringify({at:a,from:a[1],to:line[1],add:[line[1],a[1]]});
			}
			else if (a[1][0] == line[1][0] && a[1][1] == line[1][1] && a[1][2] == line[1][2]) {
				return JSON.stringify({at:a,from:a[0],to:line[0],add:[line[0],a[0]]});
			}	
			return null;
		};
		let _changePos = (vi, idx, a, line) => {
			let o = JSON.parse(JSON.stringify(vi));
			if (a[0][0] == line[0][0] && a[0][1] == line[0][1] && a[0][2] == line[0][2]) {
				o.pos = JSON.parse(JSON.stringify([line[1],a[1],vi.pos[(idx + 2) % vi.pos.length]]));
			//	this.debug.push({line:JSON.parse(JSON.stringify(line)), walls:[]});




				this.debug.push({line:JSON.parse(JSON.stringify([line[1],a[1]])), walls:[]});


			//	this.debug.push({line:JSON.parse(JSON.stringify([a[0],a[1]])), walls:[]});
	//			this.debug.push({line:JSON.parse(JSON.stringify([a[1],vi.pos[(idx + 2) % vi.pos.length]])), walls:[]});
	//			this.debug.push({line:JSON.parse(JSON.stringify([vi.pos[(idx + 2) % vi.pos.length],line[1]])), walls:[]});
...
//				this.debug.push({line:JSON.parse(JSON.stringify([vi.pos[(idx + 2) % vi.pos.length],line[1]])), walls:[]});
		...
		//		this.debug.push({line:JSON.parse(JSON.stringify(line)), walls:[]});
		//		this.debug.push({line:JSON.parse(JSON.stringify([line[1],vi.pos[(idx + 2) % vi.pos.length]])), walls:[]});


			}
			else if (a[1][0] == line[1][0] && a[1][1] == line[1][1] && a[1][2] == line[1][2]) {
				o.pos = JSON.parse(JSON.stringify([line[0],a[0],vi.pos[(idx + 2) % vi.pos.length]]));
			//	this.debug.push({line:JSON.parse(JSON.stringify(line)), walls:[]});
				this.debug.push({line:JSON.parse(JSON.stringify([a[0],line[0]])), walls:[]});


			//	this.debug.push({line:JSON.parse(JSON.stringify([a[0],a[1]])), walls:[]});
		//		this.debug.push({line:JSON.parse(JSON.stringify([line[0],vi.pos[(idx + 2) % vi.pos.length]])), walls:[]});
		//		this.debug.push({line:JSON.parse(JSON.stringify([vi.pos[(idx + 2) % vi.pos.length],a[0]])), walls:[]});
		...
		//		a[0][1] = line[0][1];
		//		a[0][2] = line[0][2];
//				this.debug.push({line:JSON.parse(JSON.stringify([line[0],a[0]])), walls:[]});
//				this.debug.push({line:JSON.parse(JSON.stringify([a[0],vi.pos[(idx + 2) % vi.pos.length]])), walls:[]});
//				this.debug.push({line:JSON.parse(JSON.stringify([vi.pos[(idx + 2) % vi.pos.length],line[0]])), walls:[]});

		//		this.debug.push({line:JSON.parse(JSON.stringify(line)), walls:[]});
			//	this.debug.push({line:JSON.parse(JSON.stringify([line[0],vi.pos[(idx + 2) % vi.pos.length]])), walls:[]});
			}
			o.area = this.getArea(o.pos);

			return o.area > 0 ? o : null;
		}

		let _normalLine = (line) => {

			if (line[0][0] >= line[1][0] && line[0][1] >= line[1][1] && line[0][2] >= line[1][2]) {
				let tmp = line[0];
				line[0] = line[1];
				line[1] = tmp;
			}
		...
		while(++i < vitems.length) {
			...
			j = -1;
			while(++j < vi.pos.length) {
				let line = [vi.pos[j],vi.pos[(j+1) % vi.pos.length]];
				let pnt = vi.pos[(j+2) % vi.pos.length];
				let found = false;

				k = -1;
				while(++k < _edges.length) {
					let a = _edges[k].line;
					if (this.util.isSamePoints(a, line)) {
						found = true;
						break;
					}
					else if (!skip) {
						if (this.isLineOverlapped(a, line)) {
							let o = null;

							if (a[0][0] == line[0][0] && a[0][1] == line[0][1] && a[0][2] == line[0][2]) {
								o = {at:a,to:line[1],add:_normalLine([a[1],line[1]])};
							}
							else if (a[1][0] == line[1][0] && a[1][1] == line[1][1] && a[1][2] == line[1][2]) {
								o = {at:a,to:line[0],add:_normalLine([a[0],line[0]])};
							}						

							if (o && !actions.find(el => { return this.isSamePoint(el.at, o.at);})) {
								actions.push(o);
							}

					//		this.debug.push({line:JSON.parse(JSON.stringify(a)), walls:[]});
					// 		let o = _changePos(vi, j, a, line);
					// 		if (o && !_adding.find(el => {
					// 			return this.util.isSamePoints(el.pos, o.pos);
					// 		})) {
					// //			_adding.push(o);
					// 		}
						...
						else if (this.isLineOverlapped(line, a)) {
							let o = null;
							
					//		this.debug.push({line:JSON.parse(JSON.stringify(line)), walls:[]});

							if (a[0][0] == line[0][0] && a[0][1] == line[0][1] && a[0][2] == line[0][2]) {
								o = {at:line,to:a[1],add:_normalLine([a[1],line[1]])};
							}
							else if (a[1][0] == line[1][0] && a[1][1] == line[1][1] && a[1][2] == line[1][2]) {
								o = {at:line,to:a[0],add:_normalLine([line[0],a[0]])};
							}						
		...
		let _debug = (pos) => {
			...
			while(++i < pos.length) {
				let line = [pos[i],pos[(i+1) % pos.length]];

				this.debug.push({line:JSON.parse(JSON.stringify(line)), walls:[]});
			}
		};

		let _doAction = (action, vi, idx, line) => {
			if (this.util.isSamePoints(action.at, line)) {
				let i = -1;

				this.debug.push({line:JSON.parse(JSON.stringify(action.at)), walls:[]});
				this.debug.push({line:JSON.parse(JSON.stringify([action.to,vi.pos[(idx + 2) % vi.pos.length]])), walls:[]});
			//	this.debug.push({line:JSON.parse(JSON.stringify([action.to,vi.pos[(idx + 2) % vi.pos.length]])), walls:[]});

			this.debug.push({line:JSON.parse(JSON.stringify([ vi.pos[idx],vi.pos[(idx + 2) % vi.pos.length]])), walls:[]});
			this.debug.push({line:JSON.parse(JSON.stringify([vi.pos[(idx + 1) % vi.pos.length],vi.pos[(idx + 2) % vi.pos.length]])), walls:[]});
	
// 				while(++i < line.length) {
// 					if (this.isSamePoint(line[i],action.from) && !this.isSamePoint(line[i],action.to)) {
// 				//		this.debug.push({line:JSON.parse(JSON.stringify([action.to, vi.pos[(idx+2) % vi.pos.length]])), walls:[]});
// 						let o = JSON.parse(JSON.stringify(vi));
// 						o.pos = JSON.parse(JSON.stringify([action.add[0],action.add[1],vi.pos[(idx + 2) % vi.pos.length]]));
// 						o.area = this.getArea(o.pos);

// 						if (o.area > 0) {
// 							line[i][0] = action.to[0];
// 							line[i][1] = action.to[1];
// 							line[i][2] = action.to[2];
// //								_debug(o.pos);
// 							_adding.push(o);
// 						}
...
// 				}
		...
		let _doActions = (vi, actions) => {
			let i = -1, j;
			let old = JSON.stringify(vi.pos);

			while(++i < vi.pos.length) {
				let line = [vi.pos[i],vi.pos[(i+1) % vi.pos.length]];

				j = -1;
				while(++j < actions.length) {
					if (_doAction(actions[j], vi, i, line)) break;
				}
			}
			if (this.getArea(vi.pos) == 0) {
				vi.pos = JSON.parse(old);
		...
		i = -1;
		while(++i < vitems.length) {
			_doActions(vitems[i], actions);
		}

		return vitems.concat(_adding);
	},
*/
	collectEdges: function () {
		let _collect = (a, b) => {
			...
			while(++i < a.length) {
				j = -1;
				while(++j < b.length) {
					if ((p = this.getSamePoints(a[i].position, b[j].position)).length == 2 && !this.findEdge(p)) {
						this.edges.push({line:p, walls:[]});
					}
				}
			}
		};
		...
		for (const [cardi, val] of Object.entries(this.walls)) {
			for (const [idx, wall] of Object.entries(val)) {
				for (const [cardi2, val2] of Object.entries(this.walls)) {
					for (const [idx2, wall2] of Object.entries(val2)) {
						if (cardi != cardi2) _collect(wall.vertices, wall2.vertices);
					}
				}
			}
		}
	...
	// 					let el2 = el.vertices[k];
	// 					let points = this.getSamePoints(el2.position, vi.pos);

	// 					if ((n = points.length) == 2) {
	// 						if (vi.cardinal == cardi && this.isGArea(el2.position) == this.isGArea(vi.pos)) {
	// 							if (linked.length == 0) {
	// 								el.vertices.push({"position":vi.pos});
	// 							}
	...
	// 						}
	// 					}
	// 					else if ((points = this.getAdjacentLine(vi.pos,el2.position)) != null || (points = this.getAdjacentLine(el2.position,vi.pos)) != null) {
	// 						if (!(vi.cardinal == cardi && this.isGArea(el2.position) == this.isGArea(vi.pos)) && !this.findEdge(points)) {
	// 							this.edges.push({line:points, walls:[]});
	// 						}
	// 					}
	// 					if(n >= 2)  {
	...
	// 			if (linked.length == 0) {
	// 				let idx = (wnum++);
	// 				wall0[vi.cardinal][idx] = {"vertices":[{"position":vi.pos}]};
	// 			}
	// 			else {
	// 				if (linked.length > 1) {
	// 					var j = 0;
	...
	// 			j = a.length;
	// 			while(--j >= 0) {
	// 				if ((p = this.getSamePoints(arr[i].position, a[j].position)).length == 2 && this.isGArea(arr[i].position) == this.isGArea(a[j].position) && !this.findEdge(p)) {
	// 			//		arr
	// 				}
	// 			}
	// 		}
	// 			if ((p = this.getSamePoints(a[i].position, b[j].position)).length == 2 && this.isGArea(a[i].position) == this.isGArea(b[j].position) && !this.findEdge(p)) {
	// 			}
	// 		}
	// 	};
	...
	// 	for (const [cardi, val] of Object.entries(this.walls)) {
	// 		for (const [idx, wall] of Object.entries(val)) {				
	// 			_collect(wall.vertices);

	// 	//		for (const [idx2, wall2] of Object.entries(this.walls[cardi])) {
	// 	//			_collect(wall.vertices, wall2.vertices);
	// 	//		}
	// 		}
	// 	}
	// }, 
/*
	collectWalls: function () {
		let _collect = (_walls, a) => {
			let b = {}, i = -1, j, p, addable;

			while(++i < a.length) {
				let A = a[i];

				addable = true;
				for (const [idx, val] of Object.entries(b)) {
					j = -1;
					while(++j < val.vertices.length) {
						let B = val.vertices[j];
						if ((p = this.getSamePoints(A.position, B.position)).length == 2 && this.isGArea(A.position) == this.isGArea(B.position) && !this.findEdge(p)) {
							val.vertices.push({"position":A.position,"slope":A.slope,"area":A.area});
							break;
						}			
					}
					if (j < val.vertices.length) {
						addable = false;
						break;
					}
				}	
				if (addable) {
					_walls[this.wnum++] = {"vertices":[{"position":A.position,"slope":A.slope,"area":A.area}]};
				}
			}
		};
		...
		for (const [cardi, val] of Object.entries(this.walls)) {
			...
			i = -1;
			while(++i < keys.length) {
				_collect(val, val[keys[i]].vertices);
			} 
		}
	}, 
	*/
	...
	collectWalls: function (vitems ) {
		...
		while(++i < vitems.length) {
			...
			if (!this.walls[vi.cardinal]) {
				this.walls[vi.cardinal] = {};
			}
			...
			for (const [cardi, value] of Object.entries(this.walls)) {
				for (const [j, el] of Object.entries(value)) {
					...
					//		let wall = {"cardi":cardi, "id":parseInt(j)};
							...
							if (vi.cardinal == cardi && this.isGArea(el2.position) == this.isGArea(vi.pos) && !this.findEdge(points)) {
								if (linked.length == 0) {
									el.vertices.push({"position":vi.pos,"slope":vi.slope,"area":vi.area});
									// linked3d.push(wall);
								}
								linked.push(j);		
							}
			...
			if(n <= 2)  {
				if (linked.length == 0) {
					let idx = (this.wnum++);
					this.walls[vi.cardinal][idx] = {"vertices":[{"position":vi.pos,"slope":vi.slope,"area":vi.area}]};
				}
				else if (linked.length > 1) {
					var j = 0;

					while(++j < linked.length) {
						this.walls[vi.cardinal][linked[0]].vertices = this.walls[vi.cardinal][linked[0]].vertices.concat(this.walls[vi.cardinal][linked[j]].vertices);
						delete this.walls[vi.cardinal][linked[j]];
					}
				}
			}
		}
	...
	isLineOverlapped: function (a, b) {
		let _isPointOnLine = (pointA, pointB, pointToCheck) => {
		...
		let _isOnLine = function (a, b, c) {
			...
			if (!_isPointOnLine(pointA, pointB, pointToCheck)) {
				return false;
			}
	...
	collectWindows: function ( offset, position ) {
		...
		let _getNearPos = (vertices, pos) => {
			...
			while(++i < vertices.length) {
				...
				while(++j < el.position.length) {
					...
					if ((el2[0] !== pos[0] || el2[2] !== pos[2]) && el2[1] == pos[1] && (d = this.distance(el2, pos)) < dist) {
						dist = d;
						ret = el2;
					}
				}
		...
		for (const [cardi, value] of Object.entries(this.walls)) {
			for (const [idx, el] of Object.entries(value)) {
				...
				if (ret.length > 0) {
					if (ret.length == 2) {
						if ((a = _getNearPos(el.vertices, ret[0])) !== null) ret.push(a);
						if ((a = _getNearPos(el.vertices, ret[1])) !== null) ret.push(a);
					}
					if (ret.length == 4) {
						let res = _getRectangle(ret);
						this.walls[cardi][this.wnum++] = {"vertices":[{"position":res,"slope":slope,"area":this.getArea(res)}],"links":[], "type":"WIN", "cardinal":cardi, "parent":idx}; // after wall divide
					}
				}
			}
		}	
	...
	shrinkRooms: function () {
		...
		let checkCrossingRoof = function(segment) {
			for (const [idx, el] of Object.entries(that.walls['UP'])) {
				if (el.boundary) {
		...
		let checkInwallEdge = function(segment) {
			for (const [cardi, value] of Object.entries(that.walls)) {
				if ( cardi != 'UP') {
					for (const [idx, el] of Object.entries(value)) {
	...
	getWallInfo: function (cardi, idx) {
		let type = this.walls[cardi][idx].type;
		let id = this.walls[cardi][idx].id;

		if (type == 'WIN' || type == 'CWALL') {
			return [
				{id:id,name:"면적: " + this.walls[cardi][idx].area.toFixed(2) + " m<sup>2</sup>"},
				{id:id,name:"둘레길이: " + this.walls[cardi][idx].circu.toFixed(2)},
				{id:id,name:"방위: " + cardi},
				{id:id,name:"기울기: " + this.walls[cardi][idx].slope.toFixed(2)},
				{id:id,name:"유형: " + type},
			];
		}
		if (cardi == 'UP') {
			return [
				{id:id,name:"면적: " + this.walls[cardi][idx].area.toFixed(2) + " m<sup>2</sup>"},
				{id:id,name:"열교길이: " + this.walls[cardi][idx].circu.toFixed(2)},
				{id:id,name:"방위: " + cardi},
				{id:id,name:"기울기: " + this.walls[cardi][idx].slope.toFixed(2)},
				{id:id,name:"유형: " + type + (this.walls[cardi][idx].innerWall ? ' (간벽)' : '')},
			];
		}
		else {
			return [
	...
	setWallId: function (rid, cardi, idx) {
		this.walls[cardi][idx].rid = rid;
		this.walls[cardi][idx].id = rid + '_' + cardi + '_' + this.walls[cardi][idx].type + '_' + this.walls[cardi][idx].snum;
	},

	getWallId: function (cardi, idx) {
		return this.walls[cardi][idx].id;
	},
	...
	sendWallData: function () {
		if (!this.debug.use) {
			parent.postMessage({"wall":this.walls,"spaces":this.spaces,"boards":this.boards,"bridges":this.bridges,"shadows":this.shadows,"snum":this.snum,"wnum":this.wnum,"tree":this.getTreeInfo(1),"tree2":this.getTreeInfo(0)},'*');
		}
	},
	...
	getSpacesInfo: function () {
		...
		let getWallsByType = (prefix, space, t) => {
			...
			while(++j < space.length) {
				let el = space[j];
				let el2 = this.walls[el.cardi][el.id];
	...
	drawInfoTree: function (id) {
		...
		if (this.debug.use && this.debug.line) {
			...
			while(++i < this.debug.line.length) {
				this.drawLine2(this.debug.line[i]);
			}
		}
...
//				this.execute( new AddObjectCommand( this, mesh ) );
	...
	setInWallTypes: function() {
		...
		// let _findPoint = (vertices, a) => {
		// 	var i = -1;

		// 	while(++i < vertices.length) {
		// 		let el = vertices[i];

		// 		if (el[0] == a[0] && el[1] == a[1] && el[2] == a[2]) {
		// 			return true;
		// 		}
		// 	}
		// 	return false;
		...
		// let _findInWindows = (pnt) => {
		// 	for (const [cardi, value] of Object.entries(this.walls)) {
		// 		for (const [idx, el] of Object.entries(value)) {
		// 			if (el.type == "WIN" && _findPoint(el.vertices, pnt)) {
		// 				return true;
		// 			}
		// 		}
		// 	}
		...
		let _isInWall = (a, b) => {
			...
			while(++i < a.length) {
				j = -1;
				while(++j < b.length) {
					let points = this.getSamePoints(a[i].position, b[j].position);

					k = -1;
					while(++k < points.length) {
						if (!arr.find(el => { return !!(el[0] == points[k][0] && el[1] == points[k][1] && el[2] == points[k][2]); })) {
							arr.push(points[k]);
						}
					}
					if (arr.length >= 4) {
		...
		let _findInWall = (vertices, cardi, i) => {
			if (cardi !== '' && this.walls[cardi]) {
				for (const [idx, el] of Object.entries(this.walls[cardi])) {
					if (idx != i && _isInWall(vertices, el.vertices)) {
						return true;
					} 
				}
			}
		...
		for (const [cardi, value] of Object.entries(this.walls)) {
			for (const [idx, el] of Object.entries(value)) {
				if (el.type != "WIN" && _findInWall(el.vertices, this.getCounterCardi(cardi), idx)) {
					el.type = "INWALL";
				}
			}
		}	
	...
	getCounterCardi: function (cardi) {
		switch(cardi) {
			case 'N':
				return 'S';
			case 'S':
	...
	// 				if (cardinal != '') {
	// 				let id = (this.wnum++);
	// 					this.walls[cardinal][id] = JSON.parse(JSON.stringify(el));
	// 					this.walls[cardinal][id].cardinal = cardinal;
	// 					this.walls[cardinal][id].dupl = {cardi:cardi,idx:idx};
	...
	buildSpaces: function() {
			...
			while(++i < this.edges.length) {
				...
				while(++j < el.length) {
					if (el[j].cardi == cardi && el[j].id == id) {
						k = -1;
						while(++k < el.length) {
							let wall2 = this.walls[el[k].cardi][el[k].id];
		...
		let _unionSpaces = (sp, spaces) => {
			var k = -1;
			let el2 = null;
			let a = this.walls[sp[0].cardi][sp[0].id].center;

			while(++k < spaces.length) {
				var l = -1;
				let el = spaces[k];
				let c = this.walls[el[0].cardi][el[0].id].center;

				while(++l < el.length) {
					let el3 = el[l];
					if (el3.cardi.indexOf('UP') < 0 && (el2 = _isWallExist(sp, el3.cardi, el3.id)) != null) {
						let b = this.walls[el2.cardi][el2.id].center;
		...
		let _collectWins = (cardi, id, sid) => {
			...
			for (const [idx, el] of Object.entries(this.walls[cardi])) {
				if (el.type == "WIN" && el.parent == id) {
					this.walls[cardi][idx].sid = sid;
					this.walls[cardi][idx].id = "S" + sid + "_" + cardi + "_WIN_" + wid++;
		...
		let _setWallId = (space, cardi, idx, snum) => {
			this.walls[cardi][idx].sid = space;
			this.walls[cardi][idx].id = 'S' + space + '_' + cardi + '_' + this.walls[cardi][idx].type + '_' + snum;
		};
		...
		let _fixableToSpace = (space, edges) => {
			...
			while(++i < space.length) {
				let el = space[i];
				let edges2 = this.walls[el.cardi][el.id].edges;
	...
	saveInfo: function () {
		return {"wall":this.walls,"spaces":this.spaces,"boards":this.boards,"bridges":this.bridges,"shadows":this.shadows,"snum":this.snum,"wnum":this.wnum};
	},
		...
		let _getBridgeKind = (kind, edge, line) => {
			...
			while(++j < edge.walls.length) {
				...
				if (cri.excludes && cri.excludes.find(el2 => {
					let wall = this.walls[el.cardi][el.id];
...
//			}
	...
	drawPoint: function (a, color) {
		const geometry = new THREE.BufferGeometry();
		geometry.setAttribute( 'position', new THREE.Float32BufferAttribute( a, 3 ) );
