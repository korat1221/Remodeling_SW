...
//
		...
		let _isValidCardi = (T1, T2, line, cardi, isBottom) => {
			if (T1 && T2 && line) {
				...
				return ((isBottom && V1.y < V2.y) || (!isBottom && V1.y > V2.y)) && 
					(
				...
				while(++j < el.length) {
					if (el[j].cardi == cardi && el[j].id == id) {
						...
						while(++k < el.length) {
							...
							if (W.cardi != "DOWN" && W.cardi.indexOf("UP") < 0 && _isValidCardi(_getAttachedTriangle(wall.vertices, line),_getAttachedTriangle(wall2.vertices, line),line,W.cardi, isBottom) && !space.find(el2 => !!(el2.cardi === W.cardi && el2.id === W.id))) {
								space.push(W);
							}
						}
					}
				...
				while(++j < el.length) {
					...
					if (edg) {
						const difference = edges.filter(
							element => !edg.find(el => {return !!(el.cardi == element.cardi && el.id == element.id);})
						);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		for (const [cardi, value] of Object.entries(this.editor.wall)) {
			for (const [j, el] of Object.entries(value)) {
				if (el.type != 'WIN' && !_isWallAttached(this.editor.spaces, cardi, j) && (sid = _getEdgesSpace(this.editor.spaces, cardi, el.edges)) !== null) {
